                  ...
                  if(i===GL.assetsActive.length) {
                    ...
                    function mkHtmlForStarredAssets (acc, asset) {
                      ...
                      var str = asset.starred
                          ? acc.str + '<div onclick="fetchview(\'interface.assets\',{user_keys: pass_args.user_keys, nonce: pass_args.nonce, asset:\'' + asset.id + '\', element: \'' + assetElementID + '\'});" class="balance">' + '<div class="icon">' + icon + '</div>' + '<h5>'+ asset.id + '</h5><div class="divider"></div><h3 class="balance balance-' + hyphenizedID + '">' + progressbar()+'</h3></div>'
                          : acc.str;

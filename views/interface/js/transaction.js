sendTransaction = function(properties) {
  ...
  p.amount = String(properties.amount).trim();
  ...
  p.elementbase = properties.elementbase;
  if (p.element !== null) {
    if (typeof p.element.getText === 'undefined') {
      p.balorig = $(p.element).attr('amount');  // running in browser
      p.balbase = $(p.elementbase).attr('amount');  // running in browser
    } else {
      p.balorig = p.element.getText();  // running in cli4ioc
      p.balbase = p.elementbase.getText();  // running in cli4ioc
    }
    if (isToken(p.asset)) {

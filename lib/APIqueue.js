...
function initialize (callbackArray) {
  ...
  global.hybrixd.APIqueueInterval = setInterval(function () {
    if (!global.hybrixd.APIqueueInitiated) {
      console.log(` [i] REST API running on: http://${global.hybrixd.restbind}:${global.hybrixd.restport}`);
      global.hybrixd.APIqueueInitiated = true;
      functions.sequential(callbackArray);
    }

    var now = Date.now();
    for (var qid in global.hybrixd.APIqueue) {
...
function restaction (properties) {
  ...
  var APIrequest = global.hybrixd.APIqueue[properties.qid];

...
function httpCall (link, host, qpath, args, method, processID, qid) {
  ...
  switch (method) {
    case 'POST':
      // to avoid an unhandled exception in NodeJS http(s) library, add content-length header
      if(args && args.headers) { args.headers['content-length']=args.data?!isNaN(args.data.length)?args.data.length:0:0; }
...
function getLastRequestTime (host) {
  if (global.hybrixd.APIhost.hasOwnProperty(host)) {
    return global.hybrixd.APIhost[host].lastRequestTime;
  } else {
    return 0;
  }
}
...
function getLink (host, APIrequest) {
  if (global.hybrixd.APIhost.hasOwnProperty(host)) {
    ...
    if (host.startsWith('ws://') || host.startsWith('wss://')) {
      ...
      let hostSplit = host.substr(6).split(':');
      let hostaddr = hostSplit[0];
      let hostport = hostSplit[1] ? Number(hostSplit[1]) : 23;
...
function call (qid, APIrequest, host, timer, now) {
  ...
  try {
    ...
    console.log(' [!] APIQueue call error for ' + host + ' : ' + e);
  }
}
...
function findAvailableHost (qid, APIrequest, timer, now) {
  ...
  if (typeof APIrequest.host === 'string') {
    ...
    if (APIrequest.host) {
      for (let i = 0; i < APIrequest.host.length; ++i) { // TODO perhaps find host with smallest LastRequestTime
...
function update (qid, APIrequest, timer, now) {
  let timestamp = APIrequest.time;
  let processID = APIrequest.pid;
  let timeout = timestamp + APIrequest.timeout;
  if (!APIrequest.busy && timeout > now && APIrequest.retries < APIrequest.retry) { // there is time and retries left, so the request is still valid
    let host = findAvailableHost(qid, APIrequest, timer, now);
    if (host) {
    ...
    scheduler.pass(processID, 1, 'Max number of API call retries exceeded');
    delete global.hybrixd.APIqueue[qid];
  } else if (timeout <= now) { // no more time left : delete the queue object and fail the process
    scheduler.pass(processID, 1, 'Request timed out (' + APIrequest.timeout + ' ms)');
    delete global.hybrixd.APIqueue[qid];
  }
}

function initialize (callbackArray) {
  let timer = 500;// TODO make a global conf value
  global.hybrixd.APIqueueInterval = setInterval(function () {
    if (!global.hybrixd.APIqueueInitiated) {
      console.log(` [i] API queue is running`);
      global.hybrixd.APIqueueInitiated = true;
      functions.sequential(callbackArray);
    }

    let now = Date.now();
    for (let qid in global.hybrixd.APIqueue) {
...
function insert (queueobject) {
  ...
  if (typeof global.hybrixd.APIqueue[qid].host === 'object' && global.hybrixd.APIqueue[qid].host.length) {
    global.hybrixd.APIqueue[qid].reqcnt = Math.floor(Math.random() * (global.hybrixd.APIqueue[qid].host.length));
  }

  ...
  var timer = 500;
    ...
    var timenow = Date.now();
      ...
      if(timeout>timenow && global.hybridd.APIqueue[qid].retries<global.hybridd.APIqueue[qid].retry) {
        if(typeof global.hybridd.APIthrottle[base]=='undefined') { global.hybridd.APIthrottle[base]=0; }
        if(global.hybridd.APIqueue[qid].next<timenow && global.hybridd.APIthrottle[base]<timenow) {
          try {
            ...
            if(typeof global.hybridd.APIqueue[qid].host=='string') {
              ...
              } else { global.hybridd.APIqueue[qid].reqcnt=0; }
            ...
            var postresult = restAPI.post(host,args, function(data,response) {
              restaction({'processID':this.processID,'data':data});
              delete global.hybridd.APIqueue[this.qid];
            }.bind({'qid':qid,'processID':processID}));
      ...
      // if timeout of queue object
        ...
        if(global.hybridd.APIqueue[qid].retries>=global.hybridd.APIqueue[qid].retry) {
...
function restaction(properties) {
  if(properties.data.code<0) { var err=1; } else { var err=properties.data['code']; }
  try { properties.data = JSON.parse(properties.data); } catch(e) {}
  // DEBUG: console.log(' ### DATA: '+JSON.stringify( hex2dec.toDec(properties.data.result) ));
  scheduler.stop(properties.processID,{'err':err,'data':properties.data});
}
...
function insert(queueobject) {
  ...
  global.hybridd.APIqueue[qid] = { 'method':'POST',
                                          'retry':3,
                                          'retries':0,
                                          'throttle':5,
                                          'timeout':15000,

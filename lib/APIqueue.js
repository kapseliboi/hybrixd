  ...
  if (typeof APIrequest.host === 'string') {
    ...
    if (APIrequest.host) {
      for (let i = 0; i < APIrequest.host.length; ++i) { // TODO perhaps find host with smallest LastRequestTime
        ...
        APIrequest.reqcnt += APIrequest.reqcnt < APIrequest.host.length - 1
          ? 1
          : 0;
...
function insert (queueobject) {
  ...
  const statuses = ['busy', 'time', 'next', 'retries', 'reqcnt'];

  if (queueobject && typeof queueobject === 'object') statuses.forEach(deleteStatusWhenQueued(queueobject)); // Remove busy,time,next,retries,reqcnt from queueobject as they should only be set by APIqueue
  global.hybrixd.APIqueue[qid] = Object.assign(queueobjectInit, queueobject);
  if (typeof global.hybrixd.APIqueue[qid].host === 'object' && global.hybrixd.APIqueue[qid].host.length) global.hybrixd.APIqueue[qid].reqcnt = Math.floor(Math.random() * (global.hybrixd.APIqueue[qid].host.length)); // if multiple hosts: randomize the first request count

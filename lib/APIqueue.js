...
function initialize (callbackArray) {
  ...
  if (!APIqueueInitiated) initializeRestApi(callbackArray);
  APIqueueInterval = setInterval(updateApiQueue(callbackArray, timer), timer); // every 'timer' milliseconds push transactions from the queue...
...
function updateQueueItem (timer, now) {
  return function (qid) {
    update(qid, APIqueue[qid], timer, now);
  };
}
...
function insertFollowUp (followUp, properties) {
  insert(followUp);
  delete APIqueue[properties.qid];
  return 0;
}
...
function restaction (properties) {
  ...
  if (!APIqueue.hasOwnProperty(properties.qid)) {
    console.log(' [!] API queue: request ' + properties.qid + ' not found');
    return 1;
  }

  let APIrequest = APIqueue[properties.qid];

...
function webSocketCall (link, host, qpath, args, method, processID, qid) {
  ...
  if (typeof link.send === 'function') {
    ...
    console.log('[!] API Queue Websocket error. link.send not a function.');
  }
}
  ...
  global.hybridd.APIqueueInterval = setInterval(function () {
    ...
    for (var qid in global.hybridd.APIqueue) {
      ...
      if (timeout > timenow && APIrequest.retries < APIrequest.retry) {
          ...
          try {
            ...
            if (APIrequest.link) { // If the link is already defined
              ...
              try {
                ...
                ws.on('open', function () {
                  console.log(' [i] WebSocket ' + this.host + ' opened.');
                  ...
                  console.log(' [i] WebSocket ' + this.host + ' closed.');
                }.bind({host})).on('error', function (code, description) {
                  console.log(' [i] WebSocket ' + this.host + ' : Error ' + code + ' ' + description);
                }.bind({host}));
              } catch (result) {
        ...
        // TODO MAYBE implement timeout catching
        //        if (APIrequest.proceedOnTimeOut) {
        //          scheduler.stop(processID, {err: 0, data: null});
        //        } else {
...
function restaction (properties) {
  ...
  if (false && APIrequest.autoproc) { // Auto proc option is to automatically resolve hybridd "two step API calls"

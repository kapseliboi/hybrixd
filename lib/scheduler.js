...
var evaluate = function evaluate (runprocess, p, err, data) {
    ...
    return ( new Function("p", "err", "data", `
      ...
      function tran(p,property,testObject,is_invalid,is_valid,data){
        ...
        if(checkVar.valid){
          jump(p,is_valid,data?data:checkVar.data); // jump to is_valid, or next
        }else{
          jump(p,is_invalid,data?data:testObject); // jump to is_invalid
        }
        return 1;
      }
      ...
      function form(p,data,factor){
        ...
        next(p,0,padFloat(Number(data),Number(factor)));
      }

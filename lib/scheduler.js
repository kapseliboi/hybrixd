...
var seqstep = function seqstep (processID, subprocesses, step) {
  ...
  if (global.hybridd.proc[processID].busy === true) { //  || global.hybridd.proc[processID].step < step
    ...
    setTimeout(() => {
      ...
      if (typeof global.hybridd.proc[processID] !== "undefined" && typeof global.hybridd.proc[processID].step !== "undefined") {
...
var finish = function(processId, data){
  return function(callbackArray){
    scheduler.stop(this.processId,{data:this.data})
    functions.sequential(callbackArray);
  }.bind({processId,data})
}

var procResult =  function(processID,request){
 return {
      "error": 0,
...
exports.finish = finish; // finishes a process
exports.result = procResult; // creates an API follow up message for a proc

...
function purge () {
  ...
  var count = 0;
  var procpurgetime = global.hybridd.schedulerProcPurgeTime || 120; // process purge time (sec): two minutes
  var now = Date.now();
  for (var processID in global.hybridd.proc) {
    if (global.hybridd.proc.hasOwnProperty(processID)) {
      var process = global.hybridd.proc[processID];

      // TODO -1 if indefinite, 0 is directly time out
      var proctimeout = process.timeout === null || typeof process.timeout === 'undefined' ? 15000 : process.timeout;
  ...
  if (count > 0) {
    console.log(' [i] Purging ' + count + ' stale processes.');
  }
}
...
exports.pass = proc.pass; // functionally stop a process

...
function executeProcesses () {
  const maxUsage = global.hybrixd.schedulerTick * global.hybrixd.schedulerMaxUsage / MAX_USAGE_DEFAULT; // limit the amount of time spent during cycle "throttle"
...
function initialize (cbArr) {
  global.hybrixd.schedulerCoreInterval = setInterval(runScheduler.bind({cbArr}), global.hybrixd.schedulerTick); // every tick perform tasks from the queue and execute all processes...
...
function purge () {
  const procPurgeTime = global.hybrixd.schedulerProcPurgeTime || PROCESS_PURGE_TIME;
  const now = Date.now();
  let count = 0;

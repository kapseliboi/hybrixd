...
/* DEPRECATED:
exports.initproc  = initproc;   // initialize a new process
exports.stopproc  = stopproc;   // functionally stop a process
exports.subqueue  = subqueue;   // queue subprocess steps
*/
exports.procpart  = procpart;   // split a process in its parent and child parts

function initialize() {
	setInterval(function() {
		var processID;
		for(processID in global.hybridd.procqueue) {
	...
	}, 50); // every 50 milliseconds perform tasks from the queue...
	setInterval(function() {
		var procpurgetime = 60; // process purge time (sec): one minute
		var processID;
		var parentID;
		for(processID in global.hybridd.proc) {
			var procinfo = procpart(processID);
			if(procinfo[1]>-1) { parentID = procinfo[0]; } else { parentID = false; }
			var proctimeout = (global.hybridd.proc[processID].timeout!=null?global.hybridd.proc[processID].timeout:15000);			
...
function subqueue(processID,subprocesses) {
  var result = true;
  var sessionID = 1;
  if(typeof subprocesses!='undefined') {
    if(subprocesses.length > -1) {
      ...
      initproc(processID, {sessionID:sessionID, subprocesses:subprocesses} );
      result = false;
    }
  }
  return result;
...
function initproc(processID,properties) {
  var sessionID = 1;
  if(typeof properties != 'undefined') {
	...
	if(processID == 0) {
		//processID = Math.abs(Math.random().toString().hashCode()).toString();
...
function stopproc(processID,properties) {
  if(typeof properties == 'undefined') { properties = {};}
  try {
    if(global.hybridd.proc[processID].stopped == null) {
      global.hybridd.proc[processID].err=(typeof properties.err!='undefined'?properties.err:0);
      global.hybridd.proc[processID].data=(typeof properties.data!='undefined'?properties.data:null);
...
function procpart(processID) {
	...
	for(i = 0; i < procsplit.length-1; i++) {
	...
	} else { procinfo[2] = procinfo[0] }
	// console.log('DUMP: '+JSON.stringify(procinfo));
	return procinfo;
}

function seqproc(processID,subprocesses) {
	var step = 0;
	var subprocess = null;
	global.hybridd.proc[processID].step = 0;
	global.hybridd.proc[processID].steps = subprocesses.length-1;
	global.hybridd.proc[processID].busy = false;
	if(DEBUG) { console.log(' [D] ('+processID+') initializing sequence processing for '+(global.hybridd.proc[processID].steps+1)+' steps...'); }
	// prepare stepping array
	for (step = 0; step < subprocesses.length; ++step) {
		initproc(processID+'.'+step);
		subprocesses[step] = subprocesses[step].replace('"processID":"'+processID+'"','"processID":"'+processID+'.'+step+'"');
		// DEBUG: console.log( 'SEQSTEP: '+subprocesses[step].replace('"processID":"'+processID+'"','"processID":"'+processID+'.'+step+'"') );
		global.hybridd.proc[processID+'.'+step].request = subprocesses[step];
	}
	seqstep(processID,subprocesses,0);	// start stepping
}

function seqstep(processID,subprocesses,step) {
	var runprocess = subprocesses[step];
	...
	if (global.hybridd.proc[processID].busy == true) {  //  || global.hybridd.proc[processID].step < step
		...
		if (global.hybridd.proc[processID+'.'+step].progress == 1) {  // && global.hybridd.proc[processID].progress < 1
			if(DEBUG) { console.log(' [D] ('+processID+') finished step '+(step+1)+' of '+(global.hybridd.proc[processID].steps+1)); }
			...
			if (global.hybridd.proc[processID].progress == 1) {
				...
				global.hybridd.proc[processID].step++;
			}
      ...
      if(step == global.hybridd.proc[processID].steps) {
        ...
        if(typeof global.hybridd.proc[prevID] != 'undefined') {
          var err = (typeof global.hybridd.proc[prevID].err != 'undefined'?global.hybridd.proc[prevID].err:1);
          var data = (typeof global.hybridd.proc[prevID].data != 'undefined'?global.hybridd.proc[prevID].data:null);
        } else { var err = null; var data = null; }
        var p = {parentID:processID,processID:processID+'.'+step};
        stop(p,err,data);
      }
		} else {
			// detailed progress reporting of subprocessing
			if(global.hybridd.proc[processID].autoprog == true) {
	...
	// initialize subprocess step
		...
		if(DEBUG) { console.log(' [D] ('+processID+') running step '+(step+1)+' of '+(global.hybridd.proc[processID].steps+1)); }
		//var prevID = processID+'.'+(step-1);
		var prevID = global.hybridd.proc[processID].subproc;
		if(typeof global.hybridd.proc[prevID] != 'undefined') {
			var err = (typeof global.hybridd.proc[prevID].err != 'undefined'?global.hybridd.proc[prevID].err:1);
			var data = (typeof global.hybridd.proc[prevID].data != 'undefined'?global.hybridd.proc[prevID].data:null);
    ...
    runprocess = subprocesses[step].replace(/peek\(/g,'peek('+subinsert+','); //.replace(/json\(/g,'json('+subinsert+',');
		if(['next','dump','logs','func','time','wait','prwt','stop','jump','test','poke','prog','pass','push','coll'].indexOf( subprocesses[step].substr(0,4) )>-1) {
    ...
    try{
      ...
      console.log(' [!] ('+processID+') proc error: '+"\n     "+runprocess+"\n     "+(e?e:'UNKNOWN ERROR!'));
		...
		setTimeout(function(){ seqstep(processID,subprocesses,global.hybridd.proc[processID].step) },200);
		// DEBUG: console.log(JSON.stringify(subprocesses[step]));
	}
}

//
// external subprocess functions
//
...
function func(p,module,funcname,funcdata,data) {
  ...
  try {
    eval('modules.module.'+module+'.main.'+funcname+'('+JSON.stringify(funcdata)+');');
  } catch(e) {
    if(DEBUG) { console.log(' [D] ('+p.parentID+') error in call (module '+module+' -> '+funcname+')'+(e?' | '+e:'')); }
    next(p,1,'[func '+module+','+funcname+']');
  }
}

//
// internal subprocess functions
//
...
function coll(p,steps) {
  ...
  var data = [];
  for(var i=global.hybridd.proc[p.parentID].step-steps;i<global.hybridd.proc[p.parentID].step;i++) {
...
// pass: wrapper for backward compatibility
function push(p,data) {
	pass(p,0,data);
}
...
function time(p,millisecs,data) {
  ...
  var loopid;
  for(var i=global.hybridd.proc[p.parentID].step; i<=global.hybridd.proc[p.parentID].steps; i++) {
    loopid = p.parentID+"."+i;
...
jump = function(p,stepto,data) {
	if(typeof data == 'undefined') { data = '[jump '+stepto+']'; }
	global.hybridd.proc[p.processID].started = Date.now();
	if(stepto == 0) { stepto = 1; }
	if(stepto < 0) {
		var previd;
		for(var i=global.hybridd.proc[p.parentID].step+stepto; i<global.hybridd.proc[p.parentID].step; i++) {
			previd = p.parentID+"."+i;
...
json = function(p,str) {
	if(DEBUG) { console.log(' [D] ('+p.parentID+') json ['+JSON.stringify(str)+'] in step '+(global.hybridd.proc[p.parentID].step+1)+' of '+(global.hybridd.proc[p.parentID].steps+1)); }
  if (str === '') str = '{}';
  eval("{{{ var p=" + str + ';;; }}}');
  return p;
};

...
var coreLoop = function(){
  ...
  var start = Date.now();
  var change = true;
  var maxUsage = 40;//TODO global.hybridd.schedulerTick*global.hybridd.schedulerMaxUsage/100; /// limit the amount of time spent during cycle "throttle"

  global.hybridd.schedulerParallelProcesses=1;

  while(Date.now()<= start + maxUsage && global.hybridd.schedulerParallelProcesses>0){  // run through all processes
    global.hybridd.schedulerParallelProcesses = 0;
    for (var processID in global.hybridd.proc) {
      if(proc.next(processID)){
...
var initialize = function initialize (callbackArray) {
  ...
  }.bind({callbackArray:callbackArray}),50);// TODO global.hybridd.schedulerTick
  ...
  for (var processID in global.hybridd.proc) {
    ...
    if (global.hybridd.proc.hasOwnProperty(processID)) {
      ...
      if (typeof global.hybridd.proc[processID] !== "undefined") {
        ...
        if (proctimeout > 0 && process.started < Date.now() - procpurgetime * 1000) {
          ...
          if (proctimeout > 0 && (process.stopped === null && process.started < Date.now() - proctimeout)) {

            if (DEBUG) {console.log(` [D] process ${processID} has timed out`);}

            process.err = 1;
            process.info = "Process timeout!";
            process.busy = false;

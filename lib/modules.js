...
function init(callbackArray) {
  //TODO Rouke check if modules are being used...
...
function initexec(modulename,command) {
  ...
  for (var engine in global.hybridd.engine) {
    if(typeof global.hybridd.engine[engine].module != "undefined" && global.hybridd.engine[engine].module == modulename) {
      // check the status of the connection, and give init feedback
      var processID = scheduler.init(0);
      var target = global.hybridd.engine[engine]; target.id = engine;
      var mode = (typeof target.mode!="undefined"?target.mode:null);
      modules.module[global.hybridd.engine[target.id].module].main.exec({processID:processID,target:target,mode:mode,factor:factor,command:command});
    }
  }
  return 1;
}

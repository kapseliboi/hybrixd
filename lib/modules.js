...
function loadModule (id, index, array) {
  ...
  global.hybrixd.logger(['info', 'modules'], 'Loading module ' + id);
  ...
  try {
    ...
    global.hybrixd.logger(['error', 'modules'], 'Failed to load module ' + id);
  ...
  if (global.hybrixd.module[id].hasOwnProperty('main') && global.hybrixd.module[id].main.hasOwnProperty('init')) {
    global.hybrixd.module[id].main.init({logs: function (...args) { global.hybrixd.logger(['info', id], ...args); }});
  }
}
...
function scanModules (id, index, array) {
  ...
  if (directoryAndFileExist) {
    modulelist.push(id);
    global.hybrixd.logger(['info', 'modules'], 'Found module ' + id + ' (javascript)');
  }
}

...
function initexec(modulename,command) {
	...
	for (var asset in global.hybridd.asset) {
      ...
      var type = (typeof asset.type!='undefined'?asset.type:null);
	...
	for (var source in global.hybridd.source) {
		if(typeof global.hybridd.source[source].module != 'undefined' && global.hybridd.source[source].module == modulename) {
			// check the status of the connection, and give init feedback
			var processID = scheduler.init(0);
			var target = global.hybridd.source[source]; target.symbol = source;
      var mode = (typeof source.mode!='undefined'?source.mode:null);
      var type = (typeof source.type!='undefined'?source.type:null);
      modules.module[global.hybridd.source[target.symbol].module].main.exec({processID:processID,target:target,mode:mode,factor:factor,command:command});  
		}
	}
}
...
function getsource(mode) {
      ...
      for (var key in global.hybridd.source) {
        if(typeof global.hybridd.source[key].mode!='undefined' && global.hybridd.source[key].mode == mode) {
          targets.push(key);
          targetscnt++;
        }
      }

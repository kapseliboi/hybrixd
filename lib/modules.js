...
function initexec (modulename, command, originalPath) { // TODO rename target to recipe
  ...
  for (var asset in global.hybridd.asset) {
    if (typeof global.hybridd.asset[asset].module !== 'undefined' && global.hybridd.asset[asset].module === modulename) {
      ...
      module[global.hybridd.asset[target.symbol].module].main.exec({processID: processID, target: target, mode: mode, factor: factor, command: command});
    }
  }
  ...
  for (var source in global.hybridd.source) {
    if (typeof global.hybridd.source[source].module !== 'undefined' && global.hybridd.source[source].module === modulename) {
      ...
      var mode = (typeof target.mode !== 'undefined' ? target.mode : null);
      module[global.hybridd.source[target.id].module].main.exec({processID: processID, target: target, mode: mode, factor: factor, command: command});
    }
  }
  // this module can provide engines (daemon)
  for (var engine in global.hybridd.engine) {
    if (typeof global.hybridd.engine[engine].module !== 'undefined' && global.hybridd.engine[engine].module === modulename) {
      // check the status of the connection, and give init feedback
      ...
      var mode = (typeof target.mode !== 'undefined' ? target.mode : null);
      module[global.hybridd.engine[target.id].module].main.exec({processID: processID, target: target, mode: mode, factor: factor, command: command});
    }
  }
  return 1;
}

    ...
    if (xpath.length === 0) {
      ...
      */
      if (typeof result === 'object') { // flat files
        ...
        if (offset && length) {
          if (result.hasOwnProperty('type') && result.type !== 'application/json' && typeof result.type !== 'undefined') {
        ...
        } else if (result.hasOwnProperty('type') && result.type !== 'application/json' && typeof result.type !== 'undefined') { // a file without  offset and length => delete result['file'];
          var filePath = '../' + result.data.replace('..', ''); // 'file://*' => file : '$HYBRIDD_HOME/*'   //TODO Make safer MAYBE warning on '..' usage?
          if (fs.existsSync(filePath)) {
            console.log('exists;' + result.data);
            result.data = fs.readFileSync(filePath).toString('utf8');
          } else {
            console.log('no exists;' + result.data);

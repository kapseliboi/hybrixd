...
asset = require("./router/asset");
command = require("./router/command");
list = require("./router/list");
help = require("./router/help");
//network = require("./router/network");
proc = require("./router/proc");
source = require("./router/source");
view = require("./router/view");
xauth = require("./router/xauth");
...
function route(request,modules) {
  ...
  if(typeof request.url === "string") {
    ...
    if (JSON.stringify(xpath) !== JSON.stringify(last_xpath) ) {// TODO reinstate && xpath[0] !== "y" && xpath[0] !== "z"
      console.log(" [i] routing request "+JSON.stringify(xpath));
    }
    last_xpath = xpath; // used to prevent double console.log on repeated calls
    ...
    if (xpath.length === 0) {
      ...
      */
      ...
      if(node){result = routeRootMap[node](request,xpath);}

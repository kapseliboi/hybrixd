...
function route(request,modules) {
	...
	if(typeof request.url == 'string') {
		xpath = request.url.split('/'); // create xpath array
		...
		// route path handling (console.log only feedbacks same route once)
		...
		if (xpath.length == 0) {
			...
			if (xpath[0] == 's' || xpath[0] == 'source') {
				result = source.process(request);
			}
			// routing views
			if (xpath[0] == 'v' || xpath[0] == 'view') {
				result = view.serve(request);
			}
			// routing xauth
			if (xpath[0] == 'x' || xpath[0] == 'xauth') {
				if(xpath.length > 1) {
					result = xauth.xauth(request);
				}
			}

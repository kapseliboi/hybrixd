    ...
    if (xpath.length === 0) {
      ...
      */
      if (typeof result === 'object') { // flat files
        ...
        } else if (result.progress === 1 && typeof result.type !== 'undefined' && result.type.startsWith('file:')) { // a file without  offset and length => delete result['file'];
          var filePath = '../' + result.data.replace('..', ''); // 'file://*' => file : '$HYBRIDD_HOME/*'   //TODO Make safer MAYBE warning on '..' usage?
          if (fs.existsSync(filePath)) {
            if (hash) {
      ...
      if (typeof xpath[0] === 'undefined' || xpath[0].length <= 1) {
        ...
        delete result.path;
      } else if (typeof result.path === 'object' && result.path !== null) {

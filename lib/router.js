...
// routing handler
function route (request, modules, pageSize, pageNum) {
  if (typeof request.url === 'string') {
    ...
    for (var i = 0; i < xpath.length; i++) {
      if (xpath[i] === '') {
        ...
        try {
          ...
          return '{"error":1, "info":"Your request "' + request.url + '" was ill formatted!"}';
        }
      }
    }
    ...
    if (xpath.length === 0) {
      ...
      if (!v.valid) {
        ...
        return JSON.stringify({help: help.help(xpath, v), error: 1, id: null, path: xpath});
      }
      ...
      /* Result should be one of the following
         - A string "file:// ..." a flat file
         ?? or {file:"xxx", type:"application/json"->header}
         - A string containging encrypted ychan data or compressed zchan data
         - An object with the results containing:
         - - error
         - - data
         - - info
         - - id
      ...
      if (typeof result === 'string' && result.startsWith('file://')) { // Handle flat files
        var fileName = '../modules' + result.substr(7); // 'file://*' => file : '$HYBRIDD_HOME/modules/*'
        // TODO read file
        // TODO use pageSize, pageNum
      }
    }

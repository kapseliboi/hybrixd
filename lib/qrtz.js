...
let confFile;
if (ops.conf) {
  confFile = resolveFilePath(ops.conf);
} else {
...
function interactiveMode (data) {
  ...
  const errorCallback = (errorMessage, errorCode) => {
    console.error(' [!] Error ' + (errorCode || 1) + ' : ' + errorMessage);
    process.stdout.write(prompt);
  };
  rl.on('line', function (line) {
    if (line === 'q' || line === 'quit' || line === 'exit') {
      ...
      execute(script, [], dataCallback, errorCallback, progressCallback);
    }
  });
}
...
const pipeInteractive = hasPipeInput && (ops.interactive || typeof ops.args === 'undefined') && !ops.source;
...
if (interactive) {
  ...
  if (fs.existsSync(source)) {
    ...
    console.error('qrzt: ' + source + ': No such file or directory');
    process.exit(1);
  }
} else if (pipeScript) {
  process.stdin.on('end', function () {

...
function process(request,xpath) {
  var result = null;
	...
	if (xpath.length == 1) { 
		...
		if (xpath[1]=='peek' && request.sessionID==1) {       // only root
			if (xpath.length==3) {
				result = procdata(xpath[2],request.sessionID,true);
			}
		} else {
			if (xpath[1]=='queue' && request.sessionID==1) {    // only root
				if (xpath.length==3) {
...
function proclist(sessionID) {
	...
	for (var key in global.hybridd.proc) {
      ...
      if(sessionID==1 || sessionID==global.hybridd.proc[key.split('.')[0]].sid) {
        sproccnt++;
      }
		}
	}

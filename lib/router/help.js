...
function getFileTypeData (request, xpath) {
  if (xpath[1] === 'api') {
    ...
    return {type: 'file:application/x-font-woff'};
  } else if (xpath[1].includes('.ttf')) {
    return {type: 'file:application/x-font-ttf'};
  } else if (xpath[1].includes('.eot')) {
    return {type: 'file:application/vnd.ms-fontobject'};
  } else if (xpath[1].includes('.png')) {
    return {type: 'file:image/png'};
  } else if (xpath[1].includes('.svg')) {
    return {type: 'file:image/svg+xml'};
  } else if (xpath[1] === 'hybrix-lib.web.js') {
    return {data: '../interface/hybrix-lib.web.js', type: 'file:application/javascript'};
  } else if (xpath[1] === 'docs.js') {
    return {data: '../docs/' + xpath[1], type: 'file:application/javascript'};
  } else if (xpath[1] === 'codemirror.js' || xpath[1] === 'javascript.js') {
    return {data: '../docs/' + xpath[1], type: 'file:application/javascript'};
  } else if (xpath[1].includes('.css')) {
    return {type: 'file:text/css'};
  } else if (fs.existsSync('../docs/' + xpath[1] + '.html')) {
    return {data: '../docs/' + xpath[1] + '.html', type: 'file:text/html'};
  } else {
    return {data: '../docs/404.html', type: 'file:text/html'};
  }
}

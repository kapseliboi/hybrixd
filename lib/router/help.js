...
`;
  ...
  data += '<div id="console-wrapper"><div id="console-header">API Console  <span id="console-close" class="close" onclick="toggleConsole();">Show</span></div><div id="console-results"></div><div class="prompt"> >> <input class="text" type="text" onkeyup="if(event.keyCode===13){rout(event.target.value);event.target.value=\'\';}else if(event.keyCode===38 && commandIndex>0){ commandIndex--; event.target.value=commands[commandIndex];}else if(event.keyCode===40 && commandIndex<commands.length-1){ commandIndex++; event.target.value=commands[commandIndex];}"/></div></div>';

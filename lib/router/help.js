...
function apiEdit (sessionID, xpath, requiresRootAccess, elipsis, rootTree) {
  ...
  data += '<input type="submit" value="copy" onclick="copyToClipboard( \'\' + window.location.protocol + \'//\' + window.location.hostname + \':\' + window.location.port ' + varData + ');">';
  return data;
}
...
function html (rootPath, sessionID) {
  var data = `<html><header><title>hybridd : REST API Help</title></header><body><style>
body {
...
function display(result){
  ...
  document.getElementById('console-close').innerHTML='Hide';
       ...
       if(result.error === 0 ){
         ...
         if(result.hasOwnProperty('help')){r+=result.help.replace(/\`([^\`])*\`/g, (a,x)=>{
...
`;
  ...
  /*  data += '<a class="menuItem">Hello World</a> ';
  data += '<a class="menuItem">REST API</a> ';
  data += '<a class="menuItem">hybrid-lib.js</a> ';
  data += '<a class="menuItem">Hybridd</a> ';
  data += '<a class="menuItem">qrtz</a> '; */

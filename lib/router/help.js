...
function htmlNode(node,xpath, parentNode){
  if(typeof node === "string"){
    ...
    if(node.hasOwnProperty("_help")){
      data+="<p><i>"+node["_help"]+"</i></p>";
    }

    data+="<ul>";

    if(node.hasOwnProperty("_this")){
      data+="<li><code>["+xpath+"]</code> : "+node["_this"] +"</li>";
    }


    if(node.hasOwnProperty("_ref") && node["_ref"].hasOwnProperty("_list") ){
      var list = '$'+node["_ref"]["_list"].toUpperCase();
      data+="<li><a name='API"+encodeURIComponent(xpath+"/"+list)+"'> For each "+node["_ref"]["_list"]+" ("+list+")</a>" + htmlNode(node["_ref"],xpath+"/"+list)+"</li>";
    }

    for(var subNode in node){
      if(subNode.substr(0,1)!=='_'){
        data+="<li><a name='API"+encodeURIComponent(xpath)+"'>"+ subNode + "</a>" + htmlNode(node[subNode],xpath+'/'+subNode,node)+"</li>";
      }
    }
    data+="</ul>";
  }

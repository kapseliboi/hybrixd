...
function serve (request, xpath) {
  if (xpath.length >= 2) {
    if (xpath[1] === 'hybrix.png') {
      ...
      return {data: '../interface/hybrixd.interface.web.js', type: 'file:application/javascript', error: 0, id: null}; // TODO add request path and command
...
function apiEdit (sessionID, xpath, requiresRootAccess, elipsis, rootTree) {
  ...
  if (!requiresRootAccess || sessionID === 1) {
    ...
    data += '<input disabled type="submit" value="Try it" onclick="rout( \'\' ' + varData + ');"> <i>(Root only)</i>';
  }
...
<script src="./api/help/docs.js"></script>
  ...
  tree['_help'] = `<h2>Welcome to the hybrixd REST API</h2>

This Application Programming Interface can be used to retrieve information from hybrixd. The requests are formatted as forward slash ('/') separated commands.
...
<h3>Examples</h3>
<ul>
<li><code><span style="color:blue">/asset</span></code> returns the assets supported by the hybrixd node.<input type="submit" value="Try it" onclick="rout('/asset');"> </li>
<li><code><span style="color:blue">/asset/btc/balance/32FCGFdRGeho2HBeWQPaAYawJfPqHHvsCJ</span></code> returns the balance of the Bitcoin address 32FCGFdRGeho2HBeWQPaAYawJfPqHHvsCJ.<input type="submit" value="Try it" onclick="rout('/asset/btc/balance/32FCGFdRGeho2HBeWQPaAYawJfPqHHvsCJ');"></li>
<li><code>/command/reload</code> reload hybrixd and reinitialize modules and recipes.<input type="submit" disabled value="Try it" onclick="rout('/asset');"><i>(Root only)</i></li>
<li><code><span style="color:green">/proc</span></code> returns the list of processes running on the hybrixd node.<input type="submit" value="Try it" onclick="rout('/proc');"></li>
<li><code><span style="color:green">/proc/1543921378626236</span></code> returns the progress and result of process 1543921378626236.
</ul>
...
<p><code><span style="color:blue">/asset/btc/balance/32FCGFdRGeho2HBeWQPaAYawJfPqHHvsCJ</span></code><input type="submit" value="Try it" onclick="rout('/asset/btc/balance/32FCGFdRGeho2HBeWQPaAYawJfPqHHvsCJ');"></p>

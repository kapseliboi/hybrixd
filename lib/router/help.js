...
function getFileTypeData (request, xpath) {
  ...
  } else if (xpath[1].includes('.js')) {
    return {type: 'file:application/javascript'}; // TODO add request path and command
...
function mkCommandDataHtmlStr (data, varData, command, tr, xpath) {
  ...
  for (let i = 0; i < command.length; ++i) {
    ...
    if (command[i].startsWith('$')) {
      ...
      let id = xpath.replace('/', '_').replace('$', '_') + '_' + j;
...
function help (xpath, validity, pretty) {
  ...
  switch (validity.status) {
    ...
    // show help for last command
      break;
    case 404:
      message = mk404message(message, xpath, validity, pretty);
      break;
    case 500:
...
function mk404message (message, xpath, validity, pretty) {
  ...
  if (dynamicItem) {
    message = dynamicMessage(message, xpath, xpathLast, ypathLast, pretty);
    ...
    message = messageCannotBeUsedAsStandAlone(message, xpath, xpathLast, ypathLast, pretty);
  } else {
    message = messageIsUnknown(message, xpath, validity, xpathLast, ypathLast, pretty);
  }
  return message;
}

function dynamicMessage (message, xpath, xpathLast, ypathLast, pretty) {
  try {
    let url = xpath.join('/');
...
function messageCannotBeUsedAsStandAlone (message, xpath, xpathLast, ypathLast, pretty) {
  ...
  return `${message}${notStandAloneHtmlStr}${optionsHtmlStr}`;
}
...
function messageIsUnknown (message, xpath, validity, xpathLast, ypathLast, pretty) {
  ...
  return `${message}${unknownUrlMessageHtmlStr} ${bestMatchingSiblingHtmlStr}${optionsHtmlStr}`;
}

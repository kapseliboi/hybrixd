...
function apiEdit (sessionID, xpath, requiresRootAccess, elipsis) {
...
function describeNode (sessionID, id, node, xpath, parentNode, expanded, noFrame, requiresRootAccess) {
  var data = '';
  ...
  if (!noFrame) {
    data += '<div class="command-header" onclick="toggleCommand(\'' + xpath + '\')">' + (xpath || '/') + '<span class="quickDescription">' + quickDescription + '</span></div><div id="' + xpath + '" ' + (expanded ? 'style="display:block"' : 'style="display:none"') + '  class="command-body">';
  } else {
    data += '<div>';
  }
  if (typeof node === 'string') {
    return data + '<p>' + parseMarkUp(node) + '</p><p>' + apiEdit(sessionID, xpath, requiresRootAccess) + '</p></div>';
  }
  ...
  if (node.hasOwnProperty('_this')) {
    data += '<p>' + apiEdit(sessionID, xpath, requiresRootAccess, node['_ellipsis']) + '</p>';
  ...
  if (node.hasOwnProperty('_ref') && node['_ref'].hasOwnProperty('_list')) {
    ...
    data += describeNode(sessionID, list, node['_ref'], xpath + '/' + list, node, false, subNodeCount <= 1, requiresRootAccess);
  }
  for (var subNode in node) {
    if (!node[subNode].hasOwnProperty('_alias') && subNode.substr(0, 1) !== '_') {
      data += describeNode(sessionID, subNode, node[subNode], xpath + '/' + subNode, node, false, subNodeCount <= 1, requiresRootAccess);
    }
  }
  // if (!noFrame) {
  data += '</div>';
...
`;
  ...
  if (xpath === '') {
    data += describeNode(sessionID, '', tree, xpath, parentNode, true, requiresRootAccess);
  } else {
    data += describeNode(sessionID, '', tree, xpath, parentNode, true, requiresRootAccess);
  }

  ...
  if(typeof node === "string"){
    ...
    for(var aliasNode in parentNode){
       if(parentNode[aliasNode].hasOwnProperty("_alias") && parentNode[aliasNode]["_alias"]===xpath.substr(1)){
          info+="<p>Alias: <code>/"+aliasNode+"</code></p>";
        }
     }

    if(node.hasOwnProperty("_access")){
      info+="<p><i>Root only.</i></p>";
    }

    if(node.hasOwnProperty("_help")){
      info+="<p><i>"+node["_help"]+"</i></p>";
    }

    info+="<ul>";

    if(node.hasOwnProperty("_this")){
      info+="<li><code>["+xpath+"]</code> : "+node["_this"] +"</li>";
    }


    if(node.hasOwnProperty("_ref") && node["_ref"].hasOwnProperty("_list") ){
      var list = '$'+node["_ref"]["_list"].toUpperCase();
      info+="<li><a name='API"+encodeURIComponent(xpath+"/"+list)+"'> For each "+node["_ref"]["_list"]+" ("+list+")</a>" + htmlNode(node["_ref"],xpath+"/"+list)+"</li>";
    }

    for(var subNode in node){
      if(subNode.substr(0,1)!=='_'){
        info+="<li><a name='API"+encodeURIComponent(xpath)+"'>"+ subNode + "</a>" + htmlNode(node[subNode],xpath+'/'+subNode,node)+"</li>";
      }
    }
    info+="</ul>";
  }
...
function html(){
  ...
  for(var node in tree){
    if(!tree[node].hasOwnProperty("_alias")){
      info+="<li><a href='#"+encodeURIComponent("API/"+node)+"'>"+node+"</a></li>";
    }
  }
  info+="</ul>";
  ...
  for(var node in tree){
    if(!tree[node].hasOwnProperty("_alias")){
      info+="<h1><a name='"+encodeURIComponent("API/"+node)+"'>/"+node+"</a></h1>";
      info+= htmlNode(tree[node],"/"+node,tree);
    }
  }

  //todo list with anchors
  return {info:info,error:0,id:null};
}
  ...
  case 404:
    ...
    if (ypath_last && ypath_last.hasOwnProperty("_ref") ){
      if(ypath_last["_ref"].hasOwnProperty("_list")){
        ...
        if(list==="asset"){
          dynamic_item = global.hybridd.asset.hasOwnProperty(xpath_last);
        }else if(list==="source"){
          dynamic_item = global.hybridd.source.hasOwnProperty(xpath_last);
        }else if(list==="proc"){
          var processID = xpath_last.split(".")[0];
          if(global.hybridd.proc.hasOwnProperty(processID)){
            //TODO check session Id??
            dynamic_item=true;
          }
        }else if(list==="view"){
          dynamic_item = global.viewList.indexOf(xpath_last)>=0;
        }
      }
    }
  ...
  for(var i=0, len = xpath.length;i<len;++i){
    ...
    if(it !== null && typeof it === 'object'){
      ...
      if(it.hasOwnProperty("_ref")){ // Next xpath node is part of a dynamic list
        ...
        if(list === "asset"){ // Check if it's a valid asset
          if(global.hybridd.asset.hasOwnProperty(xpath[i])){flag_found_ref=true;}
        }else if(list === "source"){ // Check if it's a valid source
          if(global.hybridd.source.hasOwnProperty(xpath[i])){flag_found_ref=true;}
        }else if(list === "proc"){ // Check if it's a valid proc

          var processID = xpath[i].split(".")[0];
          console.log("processID",processID, global.hybridd.proc.hasOwnProperty(processID));
          if(global.hybridd.proc.hasOwnProperty(processID)){
            console.log("Found");
            if(sessionID!==1 && sessionID!==global.hybridd.proc[processID].sid){ //Check if process owner or root
              return {valid:false,ypath:ypath,status:403}; // Signal Forbidden
            }else{
              flag_found_ref=true;
            }
          }
        }else if(list==="view"){
          if(global.viewList.indexOf(xpath_last)>=0){flag_found_ref=true;}

        }else{ // Reference list does not exist
          return {valid:false,ypath:ypath,status:500, msg:"Reference List '"+list+"' is not defined."};
        }
        if(flag_found_ref){
          it=it["_ref"];
        }

...
function describeNode (node, xpath, parentNode) {
  ...
  if (node.hasOwnProperty('_this')) {
    data += "<p class='this'><a href='/api/help/" + (xpath.replace(/\$\w*/g, '_ref')) + "'>" + xpath + '</a></p><p>' + apiEdit(xpath, true) + '</p><p>' + parseMarkUp(node['_this'] + '</p>');
  ...
  if (node.hasOwnProperty('_ref') && node['_ref'].hasOwnProperty('_list')) {
    ...
    data += "<dt><a href='/api/help/" + (xpath.replace(/\$\w*/g, '_ref')) + "'>" + list + '</a> For each ' + node['_ref']['_list'] + ' (<code>' + list + '</code>)</dt><dd>' + describeNode(node['_ref'], xpath + '/' + list, node) + '</dd>';
  }
  //
  for (var subNode in node) {
    if (!node[subNode].hasOwnProperty('_alias') && subNode.substr(0, 1) !== '_') {
      data += "<dt><a name='API" + encodeURIComponent(xpath + '/' + subNode) + "'><a href='/api/help/" + (xpath.replace(/\$\w*/g, '_ref')) + '/' + subNode + "'>" + subNode + '</a></a></dt><dd>' + describeNode(node[subNode], xpath + '/' + subNode, node) + '</dd>';
    }
  }
  if (node.hasOwnProperty('_this')) {
    data += '</dl>';
...
function display(result){
  ...
  document.getElementById('console-close').innerHTML='Hide';
       ...
       if(result.error === 0 ){
         ...
         if(result.hasOwnProperty('help')){r+=result.help;}
...
`;
  ...
  data += '<div id="console-wrapper"><div id="console-header">API Console  <span id="console-close" class="close" onclick="toggleConsole();">Show</span></div><div id="console-results"></div><div class="prompt"> >> <input class="text" type="text" onkeyup="if(event.keyCode===13){ commands.push(event.target.value); commandIndex = commands.length; rout(event.target.value);event.target.value=\'\';}else if(event.keyCode===38 && commandIndex>0){ commandIndex--; event.target.value=commands[commandIndex];}else if(event.keyCode===40 && commandIndex<commands.length-1){ commandIndex++; event.target.value=commands[commandIndex];}"/></div></div>';
  ...
  for (var i = 0; i < rootPath.length; ++i) {
    ...
    if (node === '_ref') {
      ...
      data += '<li><a href="/api/' + rootPath.slice(0, i + 1).join('/') + '">' + list + '</a><ul>';
    } else {
      if (i > 0) {
        xpath += '/' + node;
      }
      data += '<li><a href="/api/' + rootPath.slice(0, i + 1).join('/') + '">' + node + '</a><ul>';
    }
  }
    ...
    case 404:
      ...
      if (dynamic_item) {
        ...
        if (ypath_last['_ref'] && ypath_last['_ref'].hasOwnProperty('_ref') && ypath_last['_ref']['_ref'].hasOwnProperty('_list')) { // Dynamic list
          options += ' - Specify a value for : /' + url + '/$' + ypath_last[xpath_last]['_ref']['_ref']['_list'].toUpperCase() + '<br/>';
        }
        for (var sibling in ypath_last['_ref']) {
          if (sibling.substr(0, 1) !== '_') {
            options += ' - /' + url + '/' + sibling + "' : " + getHelpMessage(ypath_last['_ref'][sibling]) + '<br/>';
          }
        }
        if (options !== '') {
        ...
        if (ypath_last[xpath_last] && ypath_last[xpath_last].hasOwnProperty('_ref') && ypath_last[xpath_last]['_ref'].hasOwnProperty('_list')) { // Dynamic list
          options += ' - Specify a value for : /' + url + '/$' + ypath_last[xpath_last]['_ref']['_list'].toUpperCase() + '<br/>';
        }
        for (var sibling in ypath_last[xpath_last]) {
          if (sibling.substr(0, 1) !== '_') {
            options += ' - /' + url + '/' + sibling + "' : " + getHelpMessage(ypath_last[xpath_last][sibling]) + '<br/>';
          }
        }
        if (options !== '') {
          message += 'Available options are:<br/>' + options;

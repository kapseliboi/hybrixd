...
function describeNode(node,xpath, parentNode){
  ...
  if(node.hasOwnProperty("_this")){
    data+="<p><a href='/help/"+(xpath.replace(/\$\w*/g, "_ref"))+"'>" + xpath + "</a></p><p>"+apiEdit(xpath,true)+"</p><p>"+parseMarkUp(node["_this"]+"</p>")
  ...
  if(node.hasOwnProperty("_ref") && node["_ref"].hasOwnProperty("_list") ){
    ...
    data+="<dt><a name='API"+encodeURIComponent(xpath+"/"+list)+"'><a href='/help/"+(xpath.replace(/\$\w*/g, "_ref"))+"'>"+list+"</a> For each "+node["_ref"]["_list"]+" (<code>"+list+"</code>)</a></dt><dd>" + describeNode(node["_ref"],xpath+"/"+list,node)+"</dd>";
  }

  for(var subNode in node){
    if(!node[subNode].hasOwnProperty("_alias") && subNode.substr(0,1)!=='_'){
      data+="<dt><a name='API"+encodeURIComponent(xpath)+"'></a><a href='/help/"+(xpath.replace(/\$\w*/g, "_ref"))+"/"+subNode+"'>" + subNode + "</a></dt><dd>" + describeNode(node[subNode], xpath+'/'+subNode,node)+"</dd>";
    }
  }
  if(node.hasOwnProperty("_this")){
    data+="</dl></dd>";

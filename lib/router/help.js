...
function getFileTypeData (request, xpath) {
  if (xpath[1] === 'api') {
    ...
    return {data: '../interface/hybrix-lib.web.js', mime: 'file:application/javascript'};
  } else if (xpath[1] === 'docs.js') {
    return {data: '../docs/' + xpath[1], mime: 'file:application/javascript'};
  ...
  for (let i = 0; i < command.length; ++i) {
    ...
    if (command[i].startsWith('$')) {
      ...
      const refList = tree._options instanceof Array
        ? tree._options
        : router.refList(name.toLowerCase());
...
function mk404message (xpath, validity, pretty) {
  ...
  const dynamicItem = ypathLast &&
    ...
    ? router.isValidRef(ypathLast['_ref'], xpathLast)
    : false;

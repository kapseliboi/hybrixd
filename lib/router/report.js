...
function exec (xpath, format, mime) {
  const sessionID = 1;
  const processID = process.create({sessionID, path: xpath, command: ['exec'], steps: ['time 3000000', 'wait', 'done']});
  ...
  test.runTests(symbolsToTest, hybrix, host,
    results => {
      ...
      process.stop(processID, 0, data);
    },
    progress => process.prog(processID, progress)
  );

  return process.getInfo(processID, sessionID);
}

...
function render (type,color, line) {
  if (type === 'html') {
    if (line.startsWith('@')) {
      return line;
    } else {
      return '<b>' + line.substr(0, 4) + '</b>' + line.substr(4).replace(labelRegex, (a, b) => ('<a class="reference" href="#' + b + '">@' + b + '</a>')).replace(variableRegex, (a, b) => ('<span class="variable">$' + b + '</span>'));
    }
  } else {
...
function header (type, pid) {
  if (type === 'html') {
    return `<table><tr><td colspan="3">pid: ${pid}</td>`;
  } else {
    return `pid: ${pid}\n`;
  }
}
...
function row (type, label, pid, prevPid, nextPids, started, progress, pdata, maxDepth, fail) {
  if (type === 'html') {
    if (label.startsWith('@')) {
      ...
      const pDataStr = (JSON.stringify(pdata) || '').replace(/</g, '&lt').replace(/>/g, '&gt');
      return `<tr class="${started ? 'started' : 'notStarted'} ${fail ? 'fail' : ''}">
          ...
          <td>${symbols(type, pid, prevPid, nextPids)}&nbsp;${render(type,'',label)}</td>
          <td class="data" title='${pDataStr}'>${pDataStr.substr(0, 40) + (pDataStr.length > 40 ? '...' : '')}</td>
        </tr>`;
    }
  } else {
    if (label.startsWith('@')) {
      ...
      return `${color}${clamp('.' + pid.split('.').slice(1).join('.'), maxDepth)}${color}${s +' '+render(type,color,clamp(label, 50-s.length))} ${color}${clamp(pDataStr, 30)}${noColor}\n`;
    }
  }
}
...
function display (type, data) {
  ...
  let output = header(type, Object.keys(procs)[0]);
  let prevPid;
  let i = 0;
  for (let pid in procs) {
    ...
    const fail = process.started !== null && process.err !== null && process.err !== 0 && typeof process.err !== 'undefined';

...
function processZ (request, xpath) {
  if (xpath.length === 4) { // GET must carry payload, or '0' for POST
    try {
      ...
      request.sessionID = xpath[1];
      request.nonce = xpath[2];
      var response = route.route(request);
      var result = UrlBase64.safeCompress(xcrypt(xpath[1], xpath[2], LZString.compressToEncodedURIComponent(JSON.stringify(response))));
      return result;
    } catch (err) {
      ...
      // 3rd argument contains crypt_hex
        ...
        if (crypt_msg) {
          ...
          // store nonce2 provided by the client
            ...
            var nonce1_dec = new Decimal(hex2dec.toDec(global.hybrixd.xauth.session[session_hexkey].nonce1));
            var nonce2_dec = new Decimal(hex2dec.toDec(global.hybrixd.xauth.session[session_hexkey].nonce2));
            ...
            var client_session_pubkey = global.hybrixd.xauth.session[session_hexkey].client_session_pubkey;
            var server_session_seckey = global.hybrixd.xauth.session[session_hexkey].server_session_seckey;
...
function xcrypt (session_hexkey, session_step, txtdata) {
  ...
  if (typeof global.hybrixd.xauth.session[session_hexkey].server_session_seckey !== 'undefined' && !global.hybrixd.xauth.session[session_hexkey].invalid) {
    var server_sign_pubkey = global.hybrixd.xauth.session[session_hexkey].server_session_pubsign;
    var client_session_pubkey = nacl.from_hex(global.hybrixd.xauth.session[session_hexkey].client_session_pubkey);
    var server_session_seckey = nacl.from_hex(global.hybrixd.xauth.session[session_hexkey].server_session_seckey);
    // calculate current session nonce from nonce1 + nonce2 + step
    var nonce1_dec = new Decimal(hex2dec.toDec(global.hybrixd.xauth.session[session_hexkey].nonce1));
    var nonce2_dec = new Decimal(hex2dec.toDec(global.hybrixd.xauth.session[session_hexkey].nonce2));
    var step_dec = new Decimal(session_step);
...
function xplain (session_hexkey, session_step, encdata) {
  // decrypts the datagram sent to ychan
  // (mirrored for client in hybrixdcall.js )
  if (!global.hybrixd.xauth.session[session_hexkey].invalid) {
    if (typeof global.hybrixd.xauth.session[session_hexkey].server_session_seckey !== 'undefined' && typeof encdata !== 'undefined' && encdata) {
      // get payload
      ...
      // dump session
      global.hybrixd.xauth.session[session_hexkey].invalid = 1;
      payload = null;
    }
    return payload;
  } else {

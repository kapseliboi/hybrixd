...
let Decimal = require('../../common/crypto/decimal.js');
  ...
  return function (request, xpath) {
    if (xpath.length === 4) { // GET must carry payload, or '0' for POST
      try {
        ...
        console.log('[!] x chan error 1 : ', err);
        return ''; // Silent Error
      }
    } else {
      console.log('[!] x chan error 2.');
      return ''; // Silent Error
    }
  };
}
...
function createSession (sessionHexKey, secretsJson, sessionStep) {
  ...
  const nonceConvert = new Decimal( sessionData.nonce_constr.toFixed(0).toString() ).toHex().toString('hex');
...
function getPayload (hexKey, cryptData, sessionStep, sessionData, naclFn, decodingFn) {
  let nonceConvert = new Decimal( sessionData.nonce_constr.toFixed(0).toString() ).toHex().toString('hex'); // Agent725 bugfix: cutoff disabled to ensure working nonce   // convert nonce_construct integer string back into hex
...
function getSessionData (hexKey, sessionStep) {
  const nonce1Dec = new Decimal( hexKey.nonce1 ).toFixed(0); // simplified
  const nonce2Dec = new Decimal( hexKey.nonce2 ).toFixed(0); // simplified

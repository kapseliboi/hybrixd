...
function getProcessPayload (fn, fn2, errorLogStr) {
    ...
    if (xpath.length === 4) { // GET must carry payload, or '0' for POST
      try {
        ...
        const reqSessionData = { // do a nested route on the decoded GET/POST to return plaintext result (encrypt the result, and put in a JSON object)
          url: fn(getPayloadFromType(sessionHexKey, xpath[2], UrlBase64.safeDecompress(xpath[3]), 'xplain', hexIsValid)),
          sessionID: xpath[1],
          nonce: xpath[2]
        };
        const req = Object.assign(request, reqSessionData);
        const payloadOrNull = getPayloadFromType(sessionHexKey, xpath[2], fn2(req), 'xcrypt', credentialsAreValid);

...
function procdata (processID, sessionID, peek) {
  ...
  if (typeof global.hybridd.proc[processID] !== 'undefined' && (sessionID === 1 || sessionID === global.hybridd.proc[processID.split('.')[0]].sid)) {
    ...
    if (peek) {
      ...
      result = {error: (err || 0), info: info, id: processID, progress: process.progress, started: process.started, stopped: process.stopped, data: process.data, path: process.path, offset: process.offset, length: process.length, type: process.type};
    }
  } else {

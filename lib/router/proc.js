  ...
  if (xpath.length === 1) {
    ...
    if (xpath.length === 3) {
      if (typeof global.hybrixd.procqueue[xpath[2]] !== 'undefined') {
        result = {error: 0, id: 'proc/queue', data: 'Queue item.', data: global.hybrixd.procqueue[xpath[2]]};
      } else {
        result = {error: 1, id: 'proc/queue', data: 'The queue item specified does not exist!'};
      }
    } else {
      result = queuelist();
    }
...
function queuelist () {
  ...
  return {error: 0, id: 'proc/queue', data: 'List of queue items.', count: qcnt, data: queue};
}
...
function proclist (sessionID) {
  ...
  return {error: 0, id: 'proc', data: 'List of processes.', count: proccnt, subprocesses: sproccnt, data: procs};
}
...
function procdata (processID, sessionID, peek) {
  ...
  if (typeof global.hybrixd.proc[processID] !== 'undefined' && (sessionID === 1 || sessionID === global.hybrixd.proc[processID.split('.')[0]].sid)) {
    ...
    result = {error: 1, id: 'proc' + (peek ? '/peek' : ''), data: 'The process specified cannot be accessed!'};
  }
  return result;
}
...
function procKill (processID, sessionID) {
  if (typeof global.hybrixd.proc[processID] !== 'undefined' && (sessionID === 1 || sessionID === global.hybrixd.proc[processID.split('.')[0]].sid)) {
    ...
    return {error: 0, id: 'proc/kill', data: 'The process and child processes have been been terminated.'};
  } else {
    return {error: 1, id: 'proc/kill', data: 'The process specified cannot be accessed.'};
  }
}
...
function procPause (processID, sessionID) {
  if (typeof global.hybrixd.proc[processID] !== 'undefined' && (sessionID === 1 || sessionID === global.hybrixd.proc[processID.split('.')[0]].sid)) {
    ...
    return {error: 0, id: 'proc/pause', data: 'The process and child processes have been been paused.'};
  } else {
    return {error: 1, id: 'proc/pause', data: 'The process specified cannot be accessed.'};
  }
}
...
function procResume (processID, sessionID) {
  if (typeof global.hybrixd.procPaused[processID] !== 'undefined' && (sessionID === 1 || sessionID === global.hybrixd.procPaused[processID.split('.')[0]].sid)) {
    ...
    return {error: 0, id: 'proc/resume', data: 'The process and child processes have been been resumed.'};
  } else {
    return {error: 1, id: 'proc/resume', data: 'The paused process specified cannot be accessed.'};
  }
}
...
function procListPaused (sessionID) {
  ...
  return {error: 0, id: 'proc', data: 'List of paused processes.', count: proccnt, subprocesses: sproccnt, data: procs};
}

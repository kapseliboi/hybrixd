  ...
  if (xpath.length === 1) {
    ...
    if (xpath.length === 3) {
      if (typeof global.hybrixd.procqueue[xpath[2]] !== 'undefined') {
        result = {error: 0, id: 'proc/queue', info: 'Queue item.', data: global.hybrixd.procqueue[xpath[2]]};
      } else {
...
function procdata (processID, sessionID, peek) {
  ...
  if (typeof global.hybrixd.proc[processID] !== 'undefined' && (sessionID === 1 || sessionID === global.hybrixd.proc[processID.split('.')[0]].sid)) {
    ...
    if (peek) {
      ...
      result = {error: (err || 0), info: info, id: processID, progress: process.progress, started: process.started, stopped: process.stopped, data: process.data, path: process.path, offset: process.offset, length: process.length, hash: process.hash, type: process.type};
    }
  } else {
    result = {error: 1, id: 'proc' + (peek ? '/peek' : ''), info: 'The process specified cannot be accessed!'};
  }
  return result;
}

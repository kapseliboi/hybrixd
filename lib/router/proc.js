...
function procdata (processID, sessionID, peek) {
  if (typeof global.hybridd.proc[processID] !== 'undefined' && (sessionID === 1 || sessionID === global.hybridd.proc[processID.split('.')[0]].sid)) {
    if (peek) {
      ...
      var result = {error: (err || 0), info: info, id: processID, progress: global.hybridd.proc[processID].progress, started: global.hybridd.proc[processID].started, stopped: global.hybridd.proc[processID].stopped, data: global.hybridd.proc[processID].data, path: global.hybridd.proc[processID].path};
    }
  } else {

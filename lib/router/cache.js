...
function moduleExecOrError (recipe, xpath, sessionID, data) {
  const moduleExists = global.hybrixd.module.hasOwnProperty(recipe.module);
  return !moduleExists
    ? {error: 1, data: 'Module "' + recipe.module + '" not found!'}
...
function activateModuleExec (data, recipe, xpath, sessionID) {
  ...
  // init new process
  return scheduler.exec(data, recipe, xpath, command, sessionID);
}
...
function execOrError (recipe, xpath, sessionID, data) {
  const moduleExec = global.hybrixd.module[recipe.module];

  return !moduleExec
    ? {error: 1, data: 'Module "' + recipe.module + '" not found or disfunctional!'}
    : activateModuleExec(data, recipe, xpath, sessionID);
}

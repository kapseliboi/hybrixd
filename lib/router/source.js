...
function listSources () {
  var sources = [];
  for (var source in global.hybrixd.routetree.source) {
...
function process (request, xpath) {
  ...
  if (xpath.length === 1) {
    ...
    if (global.hybrixd.source.hasOwnProperty(sourceId) && global.hybrixd.source[sourceId].hasOwnProperty('module')) {
      var target = global.hybrixd.source[sourceId];
...
function sourceexec (target, xpath, sessionID, data) { // TODO rename target to recipe
  ...
  var result = modules.module[global.hybrixd.source[target.source].module].main.exec({
    command: command,
    processID,
    target
  });

...
var sourceexec = function sourceexec (target, lxpath, sessionID,data) {
  ...
  if (xpath.length === 1) {
    ...
    case "blockexplorer":
      ...
      if (typeof xpath[2] === "string" && xpath[2].substr(0, 1) === "*") {
        ...
        // resolve blockexplorer xpath[2] if necessary
        var resxpath = null;
        if(typeof xpath[2]==='string' && typeof global.hybridd.source[xpath[2]] === 'undefined') {
          for (var key in global.hybridd.source) {
            if(typeof global.hybridd.source[key].mode !== "undefined" && global.hybridd.source[key].mode === xpath[2]) {
              resxpath = global.hybridd.source[key].id;
            }
          }
        } else {
          resxpath = xpath[2];
        ...
        if(resxpath) {
          target = global.hybridd.source[resxpath];
        } else {
          return {
            "error": 1,
            "id": "source/*",
            "info": "An existing id or mode must be specified! Example: insight.btc/unspent/ADDRESS"
          };
        }
    ...
    if (global.hybridd.source.hasOwnProperty(target.id) && global.hybridd.source[target.id].hasOwnProperty("module")) {

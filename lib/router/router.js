...
function forbidden (it, sessionID) {
  return !(!it.hasOwnProperty('_access') || it['_access'] !== 'root' || (it['_access'] === 'root' && sessionID === 1));
}
...
function handleRequest (request) {
  ...
  if (xpath[0] !== 'y' && !(xpath[0] === 'p' && xpath[0] !== 'debug') && xpath[0] !== 'z' && routedPath !== lastRoutedPath && request.hideInLogs !== false) {
    global.hybrixd.logger(['info', 'router'], 'Routing request /' + xpath.join('/'));
  }

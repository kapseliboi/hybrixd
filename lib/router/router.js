...
function handleRequest (request) {
  ...
  let routpath = xpath.join('/');
  if ( xpath[0] !== 'y' && xpath[0] !== 'z' &&
       routpath !== global.hybrixd.last_routed
      ) {
    console.log(' [i] routing request /' + routpath);
  }
  // bit of a dirty hack to avoid repetitive logging (agent725)
  global.hybrixd.last_routed = routpath; // used to prevent double console.log on repeated calls

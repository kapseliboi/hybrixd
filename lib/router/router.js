...
let last_routed_xpath = ''; // avoid double display logging of routing
...
function handleItPath (it, sessionID, xpath) {
  ...
  for (let i = 0, len = xpath.length; i < len; ++i) {
    ...
    if (it !== null && typeof it === 'object') {
      ...
      let ellipsis = it.hasOwnProperty('_ellipsis') && it['_ellipsis'];
...
function handleRequest (request) {
  ...
  if ( xpath[0] !== 'y' && xpath[0] !== 'z' &&
       xpath !== global.hybrixd.last_routed_xpaths[0] &&
       xpath !== global.hybrixd.last_routed_xpaths[1] &&
       xpath !== global.hybrixd.last_routed_xpaths[2] &&
       xpath !== global.hybrixd.last_routed_xpaths[3]
      ) {
    console.log(' [i] routing request /' + xpath.join('/'));
  }
  // bit of a dirty hack to avoid repetitive logging (agent725)
  global.hybrixd.last_routed_xpaths[3] = global.hybrixd.last_routed_xpaths[2];
  global.hybrixd.last_routed_xpaths[2] = global.hybrixd.last_routed_xpaths[1];
  global.hybrixd.last_routed_xpaths[1] = global.hybrixd.last_routed_xpaths[0];
  global.hybrixd.last_routed_xpaths[0] = xpath; // used to prevent double console.log on repeated calls
  ...
  return !v.valid
    ...
    : getFinalResult(request, xpathOrHelp, shorthand);
}

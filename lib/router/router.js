...
function refList (list) {
  if (list === 'proc') return qrtzProcess.getProcessList(); // TODO +sessionID
...
function isValidRef (it, item) {
  ...
  if (list === 'proc') return qrtzProcess.processExists(item); // TODO use session?
...
function handleItPath (it, sessionID, xpath) {
  ...
  for (let i = 0, len = xpath.length; i < len; ++i) {
    ...
    if (it !== null && typeof it === 'object') {
      if (forbidden(it, sessionID)) return {valid: false, ypath, status: 403};
      ...
      if (it.hasOwnProperty('_ref')) { // Next xpath node is part of a dynamic list
        ...
        flagFoundRef = isValidRef(it['_ref'], xpath[i]);
...
function handleItTypes (it, xpath, ypath, sessionID) {
  ...
  else if (forbidden(it, sessionID)) return {valid: false, ypath, status: 403};
...
function showRequestInLogs (xpath, request) {
  ...
  return xpath[0] !== 'y' && !(xpath[0] === 'p' && xpath[0] !== 'debug') && xpath[0] !== 'z' && routedPath !== lastRoutedPath && request.hideInLogs !== false;
}
...
function handleRequest (request) {
  ...
  if (showRequestInLogs(xpath, request)) global.hybrixd.logger(['info', 'router'], 'Routing request /' + xpath.join('/'));
...
function handleUiResult (result, meta) {
  ...
  result.data = result.data.toString('utf8').replace(/<head>/i, `<head><script>

...
function commandCache (xpath) {
  if (xpath[2] === 'clear') {
    cache.clear();
    return {error: 0, id: 'command/cache/clear', info: 'Cache cleared.'};
  } else {
    return {error: 1, id: 'command/cache', info: 'Unknown cache command.'};
  }
}
...
function commandExec (xpath) {
  ...
  if (fs.existsSync(filePath)) { //  check if file exists
    ...
    try {
      ...
      return {error: 1, id: 'command/exec', info: 'Error parsing JSON: ' + e + 'Faulty recipe.'};
    }
    ...
    return {error: 1, id: 'command/exec', info: 'File ' + filePath + ' not found.'};
  }
}
...
function commandUserInterface (xpath) {
  ...
  if (xpath.length === 2) {
    ...
    if (userinterface === 'disabled') {
      ...
      if (global.hybrixd.uiServer.listening) {
        result = {error: 0, id: 'command/userinterface/listen', info: `User Interface is already listening on: http://${global.hybrixd.userbind}:${global.hybrixd.userport}`};
      } else {
        servers.public.listen();
        result = {error: 0, id: 'command/userinterface/listen', info: `User Interface resumed listening on: http://${global.hybrixd.userbind}:${global.hybrixd.userport}`};
      }
    } else {
    ...
    if (userinterface === 'disabled') {
      ...
      if (global.hybrixd.uiServer.listening) {
        result = {error: 0, id: 'command/userinterface/status', info: `User Interface is running on: http://${global.hybrixd.userbind}:${global.hybrixd.userport}`};
      } else {
...
function commandRestInterface (xpath) {
  ...
  if (xpath.length === 2) {
    ...
    if (restinterface === 'disabled') {
      ...
      if (global.hybrixd.restServer.listening) {
        result = {error: 0, id: 'command/restinterface/listen', info: `Rest Interface is already listening on: http://${global.hybrixd.restbind}:${global.hybrixd.restport}`};
      } else {
        servers.local.listen();
        result = {error: 0, id: 'command/restinterface/listen', info: `Rest Interface resumed listening on: http://${global.hybrixd.restbind}:${global.hybrixd.restport}`};
      }
    } else {
    ...
    if (restinterface === 'disabled') {
      ...
      if (global.hybrixd.restServer.listening) {
        result = {error: 0, id: 'command/restinterface/status', info: `Rest Interface is running on: http://${global.hybrixd.restbind}:${global.hybrixd.restport}`};
      } else {

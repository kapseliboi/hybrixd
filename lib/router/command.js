...
function commandReload (xpath) {
  ...
  if (xpath.length === 2) { // Reload everything
    ...
    functions.sequential([APIqueue.pause, scheduler.pause, conf.reload, recipes.init, modules.init, scheduler.resume, APIqueue.resume, scheduler.finish(processID, 'Reload successfully')]);
    ...
    scheduler.fire(processID, ['wait()']);
    functions.sequential([conf.reload, scheduler.finish(processID, 'Configuration reloaded successfully.')]);
    ...
    scheduler.fire(processID, ['wait()']);
    functions.sequential([APIqueue.pause, scheduler.pause, modules.init, scheduler.resume, APIqueue.resume, scheduler.finish(processID, 'Modules reloaded successfully.')]);

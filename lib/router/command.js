...
function commandReload(xpath){
  if(xpath.length === 2){ // Reload everything
    var processID = scheduler.init(0, {sessionID:0});
    scheduler.fire(processID,["wait()"]);
    functions.sequential([APIqueue.pause,scheduler.pause,conf.reload,recipes.init,modules.init,scheduler.resume,APIqueue.resume, scheduler.finish(processID,"Reload succesfull")]);
    result = scheduler.result(processID,"command/reload");
  }else if(xpath[2]==="conf"){
    var processID = scheduler.init(0, {sessionID:0});
    scheduler.fire(processID,["wait()"]);
    functions.sequential(conf.reload, scheduler.finish(processID,"Configuration reloaded succesfull"));
    result = scheduler.result(processID,"command/reload/conf");
  }else if(xpath[2]==="modules"){
    var processID = scheduler.init(0, {sessionID:0});
    scheduler.fire(processID,["wait()"]);
    functions.sequential([APIqueue.pause,scheduler.pause,modules.init,scheduler.resume,APIqueue.resume, scheduler.finish(processID,"Modules reloaded succesfull")]);
    result = scheduler.result(processID,"command/reload/modules");
  }else if(xpath[2]==="recipes"){
    var processID = scheduler.init(0, {sessionID:0});
    scheduler.fire(processID,["wait()"]);
    functions.sequential([APIqueue.pause,scheduler.pause,recipes.init,modules.init,scheduler.resume,APIqueue.resume, scheduler.finish(processID,"Recipes reloaded succesfull")]);

...
function getExecDataAndMaybeCache (cacheIdx, cacheResult, target, req, xpath) {
  let result = assetExecOrError(target, xpath, req.sessionID, req.data);
  return result;
}
...
function mkErrorObjWithData (a, pathsAssetExec) {
  const result = mkErrorObj(a);
  return Object.assign(result, {data: pathsAssetExec});
}

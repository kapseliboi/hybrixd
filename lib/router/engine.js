...
let enginelist = function enginelist (module) {
  let enginecnt = 0;
  let engine = [];
  for (let key in global.hybrixd.engine) {
...
let engineexec = function engineexec (target, xpath, sessionID, data) { // TODO rename target to recipe
...
let process = function process (request, xpath) {
  let result = null;
  let target = {};
  if (xpath.length === 1) {
    result = enginelist();

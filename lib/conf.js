...
function init (callbackArray) {
  ...
  Object.keys(global.hybridd.source).forEach(function (id) {
    if (global.hybridd.source[id].hasOwnProperty('router')) { // load router data for sources
      global.hybridd.routetree.source[id] = global.hybridd.source[id].router;
    }
  });

  if (global.hybridd.hasOwnProperty('defaultMaxListeners')) {
    require('events').EventEmitter.defaultMaxListeners = Number(global.hybridd.defaultMaxListeners);
  } else {
    require('events').EventEmitter.defaultMaxListeners = 25;
  }

  // "hostname" => ["hostname"]   ,  "[hostname1,hostname2]" =>  ["hostname1","hostname2"]
  if (global.hybridd.hostname) {
    if (global.hybridd.hostname.startsWith('[')) {
      global.hybridd.hostname = global.hybridd.hostname.substr(1, global.hybridd.hostname.length - 2).split(',');
    } else {
      global.hybridd.hostname = [global.hybridd.hostname];
    }
  } else {
    global.hybridd.hostname = ['localhost'];
  }

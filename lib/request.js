...
function createClientOptions (options) {
  return {host: options.host, port: options.port, path: options.pathPrefix + options.path, headers: options.headers};
}
...
const responder = (dataCallback, errorCallback, progressCallback) => function (response, options, waitCounter) {
  ...
  response.on('end', () => {
    ...
    if (isJson) {
      ...
      else errorCallback(`Cannot handle content-type '${response.headers['content-type']}' in cli.`);
      return;
    }

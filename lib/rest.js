...
exports.Client = function (options) {
  ...
  var Util = {
    ...
    createConnectOptions: function (connectURL, connectMethod) {
      ...
      var protocol = url.protocol.indexOf(':') === -1 ? url.protocol : url.protocol.substring(0, url.protocol.indexOf(':'));
    ...
    decodeQueryFromURL: function (connectURL) {
      ...
      var result = {};			// create decoded args from key value elements in query+
  ...
  var Method = function (url, method) {
...
var ConnectManager = {
  ...
  'proxy': function (options, callback) {
    ...
    proxyTunnel.on('connect', function (res, socket, head) {
      ...
      var clientRequest = options.clientRequest;

      var requestConfig = options.requestConfig;

      var responseConfig = options.responseConfig;								// remove "protocol" and "clientRequest" option from options, cos is not allowed by http/hppts node objects
  ...
  'normal': function (options, callback) {
    ...
    var responseConfig = options.responseConfig;

    var self = this;								// remove "protocol" and "clientRequest" option from options, cos is not allowed by http/hppts node objects
    delete options.protocol;
    delete options.clientRequest;
    delete options.requestConfig;
    delete options.responseConfig;
...
var debug = function () {
  ...
  var header = now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds() + ' [NRC CLIENT]' + arguments.callee.caller.name + ' -> ';

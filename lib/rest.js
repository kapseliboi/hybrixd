	  ...
	  createConnectOptions:function(connectURL, connectMethod) {
		...
		decodeQueryFromURL: function(connectURL) {
			var url = urlParser.parse(connectURL),
					  ...
					  keyValue,
					  result={};			// create decoded args from key value elements in query+
...
counter = 1;
	...
	var ConnectManager = {
		...
		"proxy":function(options, callback) {
			...
			proxyTunnel.on("connect",function(res, socket, head) {
				...
				var buffer=[],
					...
					responseConfig = options.responseConfig;								// remove "protocol" and "clientRequest" option from options, cos is not allowed by http/hppts node objects
				...
				var request = protocol.request(options, (res) => {
						// configure response
		...
		"normal":function(options, callback) {
				...
				self = this;								// remove "protocol" and "clientRequest" option from options, cos is not allowed by http/hppts node objects
				delete options.protocol;
				delete options.clientRequest;
				delete options.requestConfig;
				delete options.responseConfig;

...
exports.Client = function (options) {
  ...
  let Util = {
    createProxyPath: function (url) {
      const defaultHost = url.host;
      const hostWithPort = url.host.indexOf(':') === -1 ? url.hostname + ':443' : url.host;
    ...
    createProxyHeaders: function (url) {
      ...
      if (self.proxy.user && self.proxy.password) result['Proxy-Authorization'] = 'Basic ' + new Buffer([self.proxy.user, self.proxy.password].join(':')).toString('base64'); // if proxy requires authentication, create Proxy-Authorization headers
      if (!self.useProxyTunnel) { result.host = url.host; } // no tunnel proxy connection, we add the host to the headers
    ...
    overrideClientConfig: function (connectOptions, methodOptions) {
      const configs = ['requestConfig', 'responseConfig'];
      ...
      configs.forEach(function fn (config) {
        if (validateReqResOptions(methodOptions[config])) {
          util._extend(connectOptions[config], methodOptions[config]);
        }
      });

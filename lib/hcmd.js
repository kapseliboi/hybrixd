...
var req = http.get(options, function(response) { responder(response); });
...
function responder(response,wait) {
  ...
  response.on('end', function() {
    ...
    if(cmd_id == 'id') {
      ...
      if(wait<80 && progress != 1 && stopped == null && cmd_id != undefined && ['asset','source'].indexOf(cmd_id.split('/')[0])==-1 && String(firstpath).substr(1,4)!='proc') {
        wait++;
        process.stdout.write('.');
        setTimeout( function() {
          var req = http.get(options, function(response) { responder(response,wait); });
          req.on('error', function(err) {

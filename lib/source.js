...
function process(request,xpath) {
    ...
    result = {error:0, info:'List of available sources.', id:'source', count:2, data:['deterministic','blockexplorer']};
      ...
      switch(xpath[1]) {
        case 'deterministic':
          command = xpath[2];
          paths_sourceexec = ['assets','modes','hashes','hash','code'];
...
function sourceexec(target,lxpath,sessionID) {
	// init new process
...
function sourcelist(module,mode) {
	...
	return {error:0, info:'List of available sources and their modes.', id:'source/'+module, count:sourcecnt, data:functions.sortObjectByKey(source)};
}

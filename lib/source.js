...
/* global DJB2 functions modules proc scheduler:true*/
...
var sourceexec = function sourceexec (target, lxpath, sessionID) {
    ...
    if (typeof modules.module[global.hybridd.source[target.name].module] === "undefined") {
        ...
        modules.module[global.hybridd.source[target.name].module].main.exec({
            command,
            processID,
            target
        });
...
var process = function process (request, xpath) {
    ...
    if (xpath.length === 1) {
        ...
        if ([
            "deterministic",
            "blockexplorer"
        ].indexOf(xpath[1]) > -1 || Object.keys(sourcelist(xpath[1], xpath[2]).data).indexOf(command) > -1) {
        ...
        // commands for (example RPC) connected clients
            if (xpath[2] === "command" && request.sessionID) {
                ...
                var cacheVal = 12000;
                if (typeof target.cache !== "undefined") {

                    cacheVal = target.cache;

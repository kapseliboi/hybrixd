...
function subpoke (properties, scopeVars, qdata) {
  if (typeof scopeVars === 'object' && scopeVars !== null) {
    if (properties.length === 1) scopeVars[properties[0]] = qdata;
      ...
      if (!scopeVars.hasOwnProperty(properties[0])) scopeVars[properties[0]] = isNaN(Number(properties[1])) ? {} : []; // create an object or array based on keytype
      else if (scopeVars[properties[0]] instanceof Array && isNaN(Number(properties[1]))) { // if it's an array but string key is pushed, convert to object
...
function subpeek (properties, scopeVars) {
  if (typeof scopeVars === 'object' && scopeVars !== null) {
    if (properties.length === 1) return scopeVars[properties[0]];
      ...
      if (!scopeVars.hasOwnProperty(properties[0])) return undefined;
...
function pokeLocal (recipe, properties, qdata) {
  const r = loadLocalInMemory(recipe);
  if (r.e) return r;
  ...
  if (typeof content === 'string' && content !== '') {
    try {
      ...
      global.hybrixd.logger(['error', 'vars'], 'Error writing local variable to file:' + recipe.filename);
      return {e: 1, v: 'poke error: Failed writing local variable.'};
    }
  } else {
    global.hybrixd.logger(['error', 'vars'], ' Error storing local variable to file:' + recipe.filename);
    return {e: 1, v: 'poke error: Failed to store local variable.'};
  }
}
...
function poke (p, key, qdata) {
  if (typeof key === 'number') {
    ...
    key = key.replace(/\[/g, '.').replace(/\]/g, ''); // Resolve array indices '[0].property' -> '.0.property'
    ...
    if (keySplit.length === 2) { // "scope::key"
      ...
      if (isNaN(Number(key)) || key.startsWith('.')) {
        if (scope === 'local') { // local::key
          ...
          return {e: 1, v: 'poke error: Not allowed to write to configuration.'}; // TODO only SESSION root/1
...
function loadLocalInMemory (recipe) {

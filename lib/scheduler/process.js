...
const extensionsWithCache = ['png', 'css', 'ico', 'svg', 'ttf', 'woff2', 'eot'];
...
function QrtzProcessStep (qrtzStatement, qrtzProcess, step) {
  ...
  this.fork = (commandOrFunction, ydata, childReferenceID, options) => {
    ...
    if (command.length > 0 && command[0].includes('::')) { // handle calls to specifically exposed endpoints of other recipes.
      ...
      // Load other recipe
      if (global.hybrixd.asset.hasOwnProperty(scopeId)) recipe = global.hybrixd.asset[scopeId];
      else if (global.hybrixd.engine.hasOwnProperty(scopeId)) recipe = global.hybrixd.engine[scopeId];
      else if (global.hybrixd.source.hasOwnProperty(scopeId)) recipe = global.hybrixd.source[scopeId];
      else {
        this.fail(`Recipe '${scopeId}' could not be found.`);
        return 0;
      }
      // Check if other recipe exposes the requested method
      ...
      typeof recipe.expose === 'object' && recipe.expose !== null && recipe.expose.hasOwnProperty(currentId)) {
        if (recipe.expose[currentId] !== methodName && !(recipe.expose[currentId] instanceof Array) && !recipe.expose[currentId].includes(methodName)) {
          this.fail(`Recipe '${scopeId}' does not expose method '${methodName}' to '${currentId}'.`);
          return 0;
        }
      } else {
        this.fail(`Recipe '${scopeId}' does not expose method '${methodName}' to '${currentId}'.`);
  ...
  this.cach = (...args) => qrtzProcess.cach(...args);

  this.rout = (xpath, data, dataCallback, errorCallback, updateMeta = false) => {
    const sessionID = this.getSessionID();
    const result = router.route({url: xpath, sessionID, data, hideInLogs: true});
    if (result.id === 'id') { // if result is a process that has te be awaited, then read that proces until finished
      const callback = (error, data) => {
        // TODO set help and mime?
        if (error === 0) return dataCallback(data);
  ...
  this.func = (command, data) => {
    ...
    if (!modules.module.hasOwnProperty(moduleID)) {
      ...
      const fork = (path, data) => {
        if (typeof path !== 'string') return this.fail(1, 'fork: expected string path');
        else return this.fork(path.split('/'), data);
      };
      const rout = (path, data, dataCallback, errorCallback) => this.rout(path, data, dataCallback, errorCallback);
...
function purgeProcesses () {
  ...
  for (const processID in finishedProcesses) {
    ...
    const timeOut = process.getTimeOut();
    if (timeOut >= 0 && process.getStopped() < now - procPurgeTime) delete finishedProcesses[processID];
  }
}
...
function QrtzProcess (properties) {
  ...
  this.getName = () => {
    ...
    else if (recipe.hasOwnProperty('customQrtzExec')) type = 'custom';
    const id = recipe.symbol || recipe.id;
  ...
  } else if ((syncResult = qrtzFunction.getSyncFunctionName())) {
    // handle sync operations sync and serv
    const [head, parameter] = syncResult;
    queuedProcesses[processID] = this;
    progress = 0;
    step = 0;
    switch (head) {

...
function QrtzProcessStep (qrtzStatement, qrtzProcess, step) {
  ...
  this.func = (command, data) => {
    ...
    if (!modules.module.hasOwnProperty(moduleID)) {
      ...
      const peek = (key, fallback) => { const result = this.peek(key, fallback); return result.e ? fallback : result.v; };
      ...
      try {
        ...
        global.hybrixd.logger(['error', 'process'], `Failure for ${moduleID}.${head}` + e.toString());
        this.fail(1, 'Javascript module failure');
      }
    }
  };
}

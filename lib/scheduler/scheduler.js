...
function exec (data, recipe, xpath, command, sessionID) {
  ...
  if (main.hasOwnProperty(command[0])) {
    func = command[0];
  } else if (main.hasOwnProperty(recipe.module)) {
    func = recipe.module;
  }
  if (!recipesQuartz && func) { // Direct function call
    ...
    const p = {
      ...
      stop: (err, data) => {
        procErr = err;
        procData = data;
        global.hybrixd.proc[processID].data = data;
        proc.stop(processID, err, data);
      },
      pass: data => { procData = data; },
      done: data => {
        procData = data;
        global.hybrixd.proc[processID].data = data;
      ...
      poke: key => {}, // TODO
      peek: (kley, value) => {}, // TODO
      mime: mimetype => { procMime = mimetype; proc.mime(processID, mimetype); },
      // TODO help
      command,
      sessionID
    };
    data = main[func](p, data);
    procData = typeof data === 'undefined' ? procData : data; // Use direct data if available, else the set data

    if (procSync) {
      return {err: procErr, data: procData, type: procMime};// TODO help
    } else {

...
function exec (data, recipe, xpath, command, sessionID) {
  ...
  if (!global.hybrixd.module.hasOwnProperty(module)) {
    return {error: 500, data: `Module ${module} does not exist.`};
  }
  const main = global.hybrixd.module[module].main;
  ...
  if (main.hasOwnProperty(command[0])) {
    func = command[0];
  } else if (main.hasOwnProperty(module.replace(/-/g, '_'))) {
    func = module.replace(/-/g, '_');
  }
  ...
  for (let processID in global.hybrixd.proc) {
    if (typeof global.hybrixd.proc[processID] !== 'undefined') {
      ...
      if (procTimeOut >= 0 && process.stopped && process.stopped < now - procPurgeTime * 1000) {
        ...
        ++count;
      } else if (procTimeOut >= 0 && (process.stopped === null && process.initialized < now - procTimeOut)) { // set error when process has timed out on its short-term action
...
function wait (p, processIDs, millisecs, callback) {
  ...
  let permission;
  if (!global.hybrixd.proc.hasOwnProperty(p.processID) && typeof p.processID === 'string') { // check if process exists
    permission = false;
  } else {
    const mainRootProcessID = p.processID.split('.')[0];
    if (!global.hybrixd.proc.hasOwnProperty(mainRootProcessID)) {
      permission = false;
    } else {
      const sessionID = global.hybrixd.proc[mainRootProcessID].sessionID;
      const isRoot = sessionID === 1;
      if (typeof processIDs === 'string') { // single
        const rootProcessID = processIDs.split('.')[0];
        permission = global.hybrixd.proc.hasOwnProperty(processIDs) && global.hybrixd.proc.hasOwnProperty(rootProcessID) && (isRoot || sessionID === global.hybrixd.proc[rootProcessID].sessionID);
      } else if (processIDs instanceof Array) { // array
        permission = processIDs.reduce((permission, processID) => {
          const rootProcessID = processID.split('.')[0];
          return permission && global.hybrixd.proc.hasOwnProperty(processID) && global.hybrixd.proc.hasOwnProperty(rootProcessID) && (isRoot || sessionID === global.hybrixd.proc[rootProcessID].sessionID);
        }, true);
      } else { // dictionary
        permission = true;
        for (let key in processIDs) {
          ...
          if (typeof processID === 'string') {
            const rootProcessID = processIDs[key].split('.')[0];
            if (!global.hybrixd.proc.hasOwnProperty(processID) || !global.hybrixd.proc.hasOwnProperty(rootProcessID) || (!isRoot && sessionID !== global.hybrixd.proc[rootProcessID].sessionID)) {
              permission = false;
              break;
            }
          } else {
  ...
  if (permission) {
    ...
    callback(p, 1, 'read: Insufficient permissions to access processes ' + JSON.stringify(processIDs) + '.');
  }
}
...
exports.prog = proc.prog; // set progress for a processs

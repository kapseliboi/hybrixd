...
function zip (p, property, data) {
  console.log(0);
  if (typeof data !== 'object' || data === null) return {valid: false, data: data};
  ...
  if (typeof object === 'object' || object === null) {
    ...
    for (let key in object) {
      ...
      if (!(chckIndex.data instanceof Array)) return {valid: false, data: data};// must all be arrays
      if (typeof length !== 'undefined' && length !== chckIndex.data.length) return {valid: false, data: data}; // all arrays must match

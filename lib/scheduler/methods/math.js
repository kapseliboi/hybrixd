...
exports.math = ydata => function (p, equationOrOperator) {
  ...
  if (ydata instanceof Array && (functions.indexOf(equationOrOperator) !== -1 || operators.indexOf(equationOrOperator) !== -1 || typeof equationOrOperator === 'undefined')) {
    ...
    if (functions.indexOf(operator) !== -1) { // "max(1,2,3,4,5)"
      ...
      if (resultForEmpty.hasOwnProperty(operator)) {
        p.next(resultForEmpty[operator]);
        p.fail(`math : no result for empty array with operator ${operator}`);
      }
      return;
    } else { // "1+2+3+4+5"   for +, -, * , /, ..
    ...
    } else if (startsWithAnOperator(equationOrOperator)) { // "$+4" if the equation starts with a mathematical operator eg '+4' then prepend the data

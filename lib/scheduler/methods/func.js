...
/**
   ...
   * @param {String} [legacyData] - [DEPRECATED] Pass data in a deprecated manner to a func.
   * @category Process
   * @example
   * qrtz:
   * data 'hello'
...
exports.func = data => function (p, command, xdata) { // TODO: remove legacy xdata now used by ethereum module
  ...
  if (!modules.module.hasOwnProperty(moduleID)) {
    ...
    if (typeof xdata !== 'undefined') { // TODO legacy mode. DEPRECATED
      ...
      modules.module[moduleID].main[ funcname ]({pass, done, stop, fail, peek, poke, prog, mime, sessionID, command: JSON.parse(JSON.stringify(command))}, ydata);
    }
  }
};

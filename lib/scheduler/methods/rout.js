  ...
  pushToApiQueue(data, p, host, xpath, 'GET', {}, {ignoreError: true, timeout: p.getTimeOut()}, response => {
    ...
    if (typeof response === 'object') {
      if (response.id === 'id') {
        ...
        if (response.hasOwnProperty('progress') && p.hasOwnProperty('setProgress')) p.setProgress(response.progress);
        setTimeout(() => call(null, p, host, `/p/${response.data}`, onSuccess, onFailure), INTERVAL);
      } else if (response.error === 0) {
        if (response.hasOwnProperty('progress') && p.hasOwnProperty('setProgress')) p.setProgress(response.progress);
        if (response.progress === 1) return p.jump(onSuccess, response.data); else setTimeout(() => call(null, p, host, `/p/${response.id}`, onSuccess, onFailure), INTERVAL);;

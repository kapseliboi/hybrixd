...
/**
   * Test truth of a math condition and choose to jump. The condition is evalutated as in the math command on jumped accordingly.
   * @category Flow
...
exports.true = data => function (p, condition, onTrue, onFalse) {
  if (typeof condition === 'string') { // prepend data to equation that starts with a binary operator
    ...
    this.fail(p, 'true error: expected condition to be a string.');
  }
  if (math.calc(condition, p.processID) === 'true') {
    this.jump(p, isNaN(onTrue) ? 1 : onTrue || 1, data);
  } else {
    this.jump(p, isNaN(onFalse) ? 1 : onFalse || 1, data);
  }
};

...
var init = function init (processID, properties) {
  ...
  if (typeof properties !== 'undefined') {
    ...
    if (typeof properties.subprocesses !== 'undefined') {
      for (var i = 0; i < properties.subprocesses.length;) {
        var subprocess = properties.subprocesses[i];
        if (subprocess.startsWith('@')) { // define labels
          labels[subprocess.substr(1)] = i;
          properties.subprocesses.splice(i, 1);
        } else if (subprocess.startsWith('#')) { // remove comments
          properties.subprocesses.splice(i, 1);
        } else {
          ++i;
        }
    ...
    if (process.stopped === null) {
      ...
      for (var i = 0; i < processIDSplit.length; ++i) {
        ...
        if (!global.hybridd.proc.hasOwnProperty(parentProcessID)) {
          ...
          if (parentProcess.err !== 1) { // if parent already failed, do not add data to it
          ...
          /*
            processId =  xxx
            processStepId = xxx.x
            childProcessId = xxx.x.x
            childProcessStepId = xxx.x.x.x

            on error: stop all parent processes and steps
            on stop : stop only the current step and immediate parent process
            */

          if (errorNum === 0 && (i === processIDSplit.length - 1 || i === processIDSplit.length - 2)) { parentProcess.progress = 1; } // only set progress when without error, else keep progress info as error indication
          if (errorNum === 1 || i === processIDSplit.length - 1 || i === processIDSplit.length - 2) {
...
var pass = function (processID, err, data) {
  if (!global.hybridd.proc.hasOwnProperty(processID)) {
    if (DEBUG) { console.log(` [D] process ${processID} no longer exists, or cannot be stopped`); }
  } else {
    var process = global.hybridd.proc[processID];
    ...
    if (process.stopped === null) {
      ...
      process.progress = 1;
      process.stopped = now;
      if (DEBUG) { console.log(' [D] (' + processID + ') passed '); }
    } else {
      if (DEBUG) { console.log(` [D] (${processID}) process was already stopped`); }
    }
  }
...
function preprocess (statement, recipe, command, data, vars) {
  var re;
  var parsedStatement = statement;

  re = /[$][$]/g; // Search for $$
  parsedStatement = parsedStatement.replace(re, x => { return writeProperty(data); }); // Replace all "$1" with path[1]
...
var next = function (processID) {
  ...
  if (process.busy === true) { //  || global.hybridd.proc[processID].step < step
    ...
    var rootID = processID.split('.')[0];
    var rootProcess = global.hybridd.proc[rootID];
    var statement = preprocess(subprocess.request, rootProcess.recipe, rootProcess.command, data, rootProcess.vars);
    ...
    if (!parsedStatement.hasOwnProperty('error')) {
      ...
      for (var i = 0; i < parsedStatement.parameters.length; ++i) {
        var tmp;
        try {
          tmp = new Function('data', 'quartz', 'functions', 'return ' + parsedStatement.parameters[i])(data, quartz, functions);
        } catch (result) {
...
exports.pass = pass; // functionally stop a subprocess

...
const quartz = new Quartz.Quartz();
...
function setProperties (properties, labels, processID) {
...
function init (processID, properties) {
  ...
  const sessionID = properties.sessionID || 0; // default to no session
  const labels = {};
  setProperties(properties, labels, processID);

  if (processID === 0 || typeof processID === 'undefined') { // create a new process
  ...
  processes.forEach(setProcess(process, processID, properties));
...
function jump (processID, stepTo, data, errorCallback) {
  let processStepID;
  let processIDSplit = processID.split('.');
  if (processIDSplit.length % 2 === 0) { // processID is a processStepID
    processStepID = processID;
  ...
  if (global.hybrixd.proc.hasOwnProperty(processStepID) && global.hybrixd.proc.hasOwnProperty(processID)) {
    const processStep = global.hybrixd.proc[processStepID];
    const process = global.hybrixd.proc[processID];

    if (isNaN(stepTo) || stepTo === 0) {
      errorCallback('jump: illegal zero jump.');
      return;
    }
    const jumpTo = process.step + stepTo;
    if (jumpTo < 0) {
      errorCallback('jump: illegal negative jump.');
      return;
    }
    if (jumpTo > process.steps) {
      errorCallback('jump: illegal out of bounds jump.');
      return;
    }
    ...
    if (stepTo < 0) {
      for (let i = jumpTo; i < process.step; i += 1) {
        let prevProcessStepID = processID + '.' + i;
    ...
    console.log(' [!] Jump error: Process ' + processStepID + ' does not exist.');
  }
}
...
function next (processID) {
  ...
  const step = typeof process.step === 'undefined' ? 0 : process.step;

  const subprocessID = `${processID}.${step}`;
  const subprocess = global.hybrixd.proc[subprocessID];

  const prevSubprocessID = process.subproc;
  const prevSubprocess = global.hybrixd.proc[prevSubprocessID];
      ...
      if (!parsedStatement.hasOwnProperty('error')) {
        if (quartz.hasOwnProperty(parsedStatement.head)) {
          ...
          try {
            ...
            return logAndError(data, processID, `proc error: \n    ${statement}\n     ${result || 'UNKNOWN ERROR!'}`);
          }
        } else {
          return logAndError(data, processID, `Quartz does not have function "` + parsedStatement.head + '"');
        }
      } else {
        return logAndError(data, processID, `parsing error: \n     ${statement}\n     ` + parsedStatement.error);
      }
    } else {
      return logAndError(data, processID, `parsing error: \n     Process seems empty!\n     `);
    }
...
function logAndError (data, processID, errorStr) {
  console.log(` [!] (${processID}) ${errorStr}`);
  if (global.hybrixd.proc[processID].sessionID === 1) { // extensive errors for root
    error(processID, errorStr, data);
  } else {
    error(processID, 'Proc error.', data); // simple error for non root
  }
  return false;
}
...
function fire (processID, subprocesses) {
  if (typeof subprocesses !== 'undefined' && subprocesses.length > -1) {
    const rootProcessID = processID.split('.')[0];
    const rootProcess = global.hybrixd.proc[rootProcessID];
    const process = global.hybrixd.proc[processID];
    let command;
    if (typeof process !== 'undefined' && process.command !== 'undefined') {
      command = process.command;
    } else if (typeof rootProcess !== 'undefined' && typeof rootProcess.command !== 'undefined') {
      command = rootProcess.command;
    } else {
      command = [];
    }
    const sessionID = typeof rootProcess !== 'undefined' && typeof rootProcess.sessionID !== 'undefined'
      ? rootProcess.sessionID
...
function finish (processId, data) {
  return function (cbArr) {
    ...
    require('../util/sequential').next(cbArr);
  }.bind({processId, data});
}
...
function resultMessage (processID) {
  if (global.hybrixd.proc.hasOwnProperty(processID)) {
    const process = global.hybrixd.proc[processID];
    const path = process.path ? process.path.join('/') : undefined;
    return {
      'error': 0,
      'id': 'id',
      'path': path,
...
function prog (processID, progress) {
  if (global.hybrixd.proc.hasOwnProperty(processID)) {
    const process = global.hybrixd.proc[processID];
    if (process.started && !process.stopped) {
...
exports.prog = prog; // set progress for a processs

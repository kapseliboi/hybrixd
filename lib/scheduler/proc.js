...
var next = function (processID) {
  ...
  if (process.busy === true) { //  || global.hybridd.proc[processID].step < step
    ...
    if ([
      ...
      if (runprocess.slice(5) !== ')') { // runprocess = abcd(foo,bar)   -> abcd({processID:"1.0",parentID:"1"},foo,bar)
        runprocess = `${runprocess.slice(0, 5) + subinsert},${runprocess.slice(5)}`;
      } else { // runprocess = abcd() -> abcd({processID:"1.0",parentID:"1"})
        runprocess = `${runprocess.slice(0, 5) + subinsert})`;
      }
    } else {
      console.log(' [!] Unknown Quartz command: "' + subprocess.request.substr(0, 4) + '"');
      error(processID, 'Unknown Quartz command: "' + subprocess.request.substr(0, 4) + '"', data);
    }

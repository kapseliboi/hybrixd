    ...
    if (process.stopped === null) {
      ...
      for (var i = 0; i < processIDSplit.length; ++i) {
        ...
        if (!global.hybridd.proc.hasOwnProperty(parentProcessID)) {
          ...
          if (errorNum === 0 && i === processIDSplit.length - 1) { parentProcess.progress = 1; } // only set progress when without error, else keep progress info as error indication
          if (errorNum === 1 || i === processIDSplit.length - 1) {
...
function writeProperty (value) {
  if (typeof value === 'string' || typeof value === 'number') {
    ...
    return JSON.stringify(value).replace('\\', '\\\\').replace('"', '\"'); // escape slashes and double quotes
  } else {
    return 'undefined';
  }
}

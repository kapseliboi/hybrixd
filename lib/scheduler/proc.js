...
function preprocess (statement, recipe, command, data, vars, parentVars) {
  ...
  re = /\$@+/g; // Search for $@, replace with command
  parsedStatement = parsedStatement.replace(re, x => {
    return command ? JSON.stringify(command) : 'undefined';
  }); // Replace all "$@" with complete path
  ...
  parsedStatement = parsedStatement.replace(re, x => {
    return command ? command[x.substr(1)] : 'undefined';
  }); // Replace all "$1" with path[1]

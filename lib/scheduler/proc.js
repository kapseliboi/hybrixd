...
function preprocess (statement, recipe, command, data, vars, labels, step) {
  var re;
  var parsedStatement = statement;

  //  re = /[@]([_a-zA-Z][\w\-]*)/g; // Search for @labelname
  // parsedStatement = parsedStatement.replace(re, (x, label) => { return (labels[label] - step); }); // Replace all "#label" with labels[label]-step (relative jump required to go to label
...
var next = function (processID) {
  ...
  if (process.busy === true) { //  || global.hybridd.proc[processID].step < step
    ...
    var statement = preprocess(subprocess.request, rootProcess.recipe, rootProcess.command, data, rootProcess.vars, rootProcess.labels, step);

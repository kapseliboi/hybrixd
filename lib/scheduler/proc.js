...
var stop = function (processID, err, data, fullStop, type) {
    ...
    if (process.stopped === null) {
      ...
      for (var i = 0; i < processIDSplit.length; ++i) {
        ...
        if (!global.hybridd.proc.hasOwnProperty(parentProcessID)) {
          ...
          if (type === 'file') {
            parentProcess.file = data;
          }
          parentProcess.data = data;
          if (errorNum !== 0 || parentProcessID === processID || fullStop) { // On error all parent processes are also stopped

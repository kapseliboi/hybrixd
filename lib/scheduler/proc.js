...
var init = function init (processID, properties) {
  ...
  process.hook = null; // determines on failure behaviour
    ...
    if (process.stopped === null) {
      ...
      if (err && process.hook) {
        if (process.hook.hasOwnProperty('jump')) { // on error jump to the specified process
          ...
          if (process.step >= process.steps) { // if the jump jumps beyon end of program, then stop the program
            stop(processID, 0, data);
          } else {
          ...
          if (process.onFail.err) {
            process.data = process.onFail.data;

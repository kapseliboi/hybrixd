      ...
      if (err && process.hook) {
        if (process.hook.hasOwnProperty('jump')) { // on error jump to the specified process
          ...
          if (process.hook.jump > process.steps) { // if the jump jumps beyon end of program, then set the error
            ...
            var processStepID = processID + '.' + process.step;
            if (global.hybrixd.proc.hasOwnProperty(processStepID)) {
              var processStep = global.hybrixd.proc[processStepID];
  ...
  if (err) { // on error stop all parent processes and process steps
    if (processIDSplit.length % 2 === 1) { // processID is a process, seek it's active step so it can be stopped
      if (global.hybrixd.proc.hasOwnProperty(processID)) {
        var processStepID = processID + '.' + global.hybrixd.proc[processID].step;
        var k = kill(processStepID, data, now, err);
        if (k.break) { return; }
        data = k.data;
      }
    }
    for (var i = processIDSplit.length - 1; i >= 0; --i) {
      ...
      var k = kill(parentID, data, now, err);
      if (k.break) { return; }
      data = k.data;
    }
  } else {
...
var type = function (processID, newType) {
  var processStepID;
  var processIDSplit = processID.split('.');
  if (processIDSplit.length % 2 === 0) { // processID is a processStepID
    processStepID = processID;
...
function preprocess (statement, recipe, command, data, vars, parentVars) {
  ...
  parsedStatement = parsedStatement.replace(re, function (full, noRecipeId, recipeId, propertyId) {
    var scope;
    if (noRecipeId === '::' || recipeId === 'local') {
      var filePath = '../var/recipes/' + this.recipe.filename;
    ...
    } else if (global.hybrixd.asset.hasOwnProperty(recipeId)) {
      scope = global.hybrixd.asset[recipeId];
    } else if (global.hybrixd.source.hasOwnProperty(recipeId)) {
      scope = global.hybrixd.source[recipeId];
    } else if (global.hybrixd.engine.hasOwnProperty(recipeId)) {
      scope = global.hybrixd.engine[recipeId];
    } else {
  ...
  parsedStatement = parsedStatement.replace(re, (full) => {
    var propertyId = full.substr(1);
  ...
  re = /([^$]|^)([$]data|[$])([^$]|$)/g; // Search for $data or a single $
...
var next = function (processID) {
  ...
  var subprocess = global.hybrixd.proc[subprocessID];

  var prevSubprocessID = process.subproc;
  var prevSubprocess = global.hybrixd.proc[prevSubprocessID];
  ...
  if (process.busy === true) { //  || global.hybrixd.proc[processID].step < step
    ...
    var statement = subprocess.qrtz;
    if(statement) {
      var parsedStatement = parseParameter(statement, process.labels, step);
      if (!parsedStatement.hasOwnProperty('error')) {
        ...
        for (var i = 0; i < parsedStatement.parameters.length; ++i) {
          var result;
          try {
            var parameter = preprocess(parsedStatement.parameters[i], rootProcess.recipe, process.command||rootProcess.command, data, process.vars, parentVars);
...
var resultMessage = function (processID) {
  if (global.hybrixd.proc.hasOwnProperty(processID)) {
    var process = global.hybrixd.proc[processID];
    var path = process.path ? process.path.join('/') : undefined;
    return {
      'error': 0,

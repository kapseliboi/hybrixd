...
function parseObject (parameter) {
  if (parameter.startsWith('{') && parameter.endsWith('}')) {
    ...
    for (let i = 0; i < keyValuePairs.parameters.length; ++i) {
      ...
      if (keyValuePair.parameters.length === 1 && keyValuePair.parameters[0].startsWith('...')) { // handle spread operator {...{a:1}} and {...[1,2]}
      ...
      } else if (keyValuePair.parameters.length !== 2) return {error: `Expected 'key:value pair', got '${keyValuePair.parameters.join(':')}'`}; // {a:1,}  {a} or {a:b:c}
      else {
...
function convertParameter (resolvedParameter) {
  ...
  else if (resolvedParameter.startsWith('[') && resolvedParameter.endsWith(']') && !(resolvedParameter.length > 2 && resolvedParameter.startsWith('[]'))) {
    return parseArray(resolvedParameter); // [array] with exception of '[]...' for tran
  } else if (resolvedParameter.startsWith('{') && resolvedParameter.endsWith('}')) return parseObject(resolvedParameter); // {object}

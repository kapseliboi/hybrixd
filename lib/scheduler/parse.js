...
const splitStatement = function (line, labels, step) {
  ...
  if (result.parameters) {
    ...
    for (let j = 0; j < result.parameters.length; ++j) {
      result.parameters[j] = result.parameters[j].replace(regexLabelName, (x, label) => {
        if (labels.hasOwnProperty(label)) return labels[label] - step;
        else {
          global.hybrixd.logger(['error', 'parse'], `Label ${label} does not exist for "${line}".`);
          return 0;
        }
      });
    }
    ...
    for (let i = 0; i < keyValuePairs.parameters.length; ++i) {
      ...
      if (keyValuePair.parameters.length === 1) {
        if (keyValuePair.parameters[0].startsWith('...')) { // handle spread operator {...{a:1}} and {...[1,2]}
          ...
          else if (typeof itemToSpread.body === 'object' && itemToSpread.body !== null) for (const key in itemToSpread.body) body[key] = itemToSpread.body[key];

...
const splitStatement = function (line, labels, step) {
  ...
  if (result.parameters) {
    for (let j = 0; j < result.parameters.length; ++j) {
      const re = /@([_a-zA-Z][\w]*)/g; // Search for @labelname
...
function writeProperty (value, subproperties) {
  if (subproperties.length > 0) {
    if (typeof value === 'object' && value !== null) {
      ...
      global.hybrixd.logger(['error', 'parse'], `Expected object: ${value}.${subproperties.join('.')}`);
      return 'undefined';
    }
  } else {
  ...
  if (/^\d+$/.test(propertyId)) { // $1 => command parameter
    ...
    if (recipe.hasOwnProperty('vars')) {
      ...
      try {
        ...
        global.hybrixd.logger(['error', 'parse'], `Local var file corrupt for ${recipe.filename} ` + filePath);
      }
    }
    ...
    global.hybrixd.logger(['error', 'parse'], `Scope "${scopeId}" for "${scopeId}::${propertyId}" not found.`);
  }
...
function isNumber (x) {
  return x.length < 10 && // ensure that to large numbers get parsed as string
  !isNaN(x) &&
...
function convertParameter (resolvedParameter) {
  ...
  if (resolvedParameter.startsWith('"')) { // "string"
    ...
    result = parseString(resolvedParameter, '`');
  } else if (resolvedParameter.startsWith('[') && resolvedParameter.endsWith(']') && !(resolvedParameter.length > 2 && resolvedParameter.startsWith('[]'))) { // [array] with exception of [] for tran
    result = parseArray(resolvedParameter);
  } else if (resolvedParameter.startsWith('{') && resolvedParameter.endsWith('}')) { // {object}
...
function parseParameter (parameter, recipe, command, data, vars, parentVars) {

 ...
 *  stop(0,"HELP! Not found.")    // stop processing, set error to 404, and put "HELP! Not found." in main process data field
 */
 ...
 * @example
 *  type("file")    // set the data type to 'file'
 *  stop(0,"hello.txt")    // stop processing, set no error and pass the content of the hello.txt file.
 */
  this.type = function (p, xdata) {
    var ydata = typeof xdata === 'undefined' ? global.hybridd.proc[p.processID].data : xdata;
 ...
 */
  ...
  this.form = function (p, xdata, factor, convert) {
    ...
    if (typeof convert === 'undefined' || !convert || convert === 'none') {
      result = functions.padFloat(ydata, factor);
...
* @example
* fork("balance/_dummyaddress_")   // Starts a process to
  retrieve the dummy balance and continue without waiting for a reply.
 */
  this.fork = function (p, command, xdata, childId) {
    var ydata = typeof xdata === 'undefined' ? global.hybridd.proc[p.processID].data : xdata;
  ...
  /**
 * Creates a new process by calling the xpath command and wait
 ...
 */
  this.call = function (p, command, xdata) {
    var ydata = typeof xdata === 'undefined' ? global.hybridd.proc[p.processID].data : xdata;
    this.read(p, this.fork(p, command, ydata, 0));
  };
  /**
 ...
 * @param {String} key - Provide the routing path
 ...
 */
  this.load = function (p, key, value) {
 ...
 */
  this.meta = function (p, key, value) {
 ...
 * @param {Object} data -  Optional data to be passed to the new process
 * @example
 * each(["a","b","c"],"test")     //  calls
  test with data = {key=0, value="a"} and further
 * each({"a":0,"b":1,"c":2},"test")     //  calls
  test/x/y with data = {key="a", value=0} and further
 */
  this.each = function (p, container, command, xdata) {
    ...
    if (container instanceof Array) { // Array
      ...
      for (var key = 0; key < container.length; ++key) {
        processIDs.push(this.fork(p, command, {data: ydata, container: container, key: key, value: container[key]}, key));
      }
    } else { // Dictionary
      processIDs = {};
      var index = 0;
      for (var key in container) {
        processIDs[key] = this.fork(p, command, {data: ydata, container: container, key: key, value: container[key]}, index);
        ++index;
      }
    }
 ...
 * @param {Object} [data] -  Store data in this subprocess data field.
 * @example
 *  poke("fee",55)              // poke 55 into global variable "fee"
 *  poke("coolness","Wooty.")   // poke "Wooty." into variable "coolness"
 */
  this.poke = function (p, variable, pdata, xdata) {
    ...
    // test if variable is globally available in the recipe, and mutation allowed
    if(['symbol','engine','source','module','import','vars'].indexOf(variable)===-1) {
      var target = global.hybridd.proc[rootID].path[1];
      if(typeof global.hybridd.asset[target] !== 'undefined' && typeof global.hybridd.asset[target][variable] !== 'undefined') {
        global.hybridd.asset[target][variable] = pdata;
      }
      if(typeof global.hybridd.source[target] !== 'undefined' && typeof global.hybridd.source[target][variable] !== 'undefined') {
        global.hybridd.source[target][variable] = pdata;
      }
      if(typeof global.hybridd.engine[target] !== 'undefined' && typeof global.hybridd.engine[target][variable] !== 'undefined') {
        global.hybridd.engine[target][variable] = pdata;
      }
    // else store the variable in the master process
    } else {
      // store poke data in root process
      global.hybridd.proc[rootID].vars[variable] = pdata;
    }
    this.next(p, 0, ydata);
  };
  /**
 ...
 * @param {String} key - Variable name to get data from.
 * @example
 *  peek("fee")         // returns value in global var fee
 *  peek("thisnum")     // returns value in local var thisnum
 */
  // peek standalone command version stub
  this.peek = function (p, variable) {
    this.next(p, 0, this.pdat(p, variable) );
  };
  // peek inline version
  this.pdat = function (p, variable) {
    ...
    if(['symbol','engine','source','module','import','vars'].indexOf(variable)===-1) {
      var target = global.hybridd.proc[rootID].path[1];
      if(typeof global.hybridd.asset[target] !== 'undefined' && typeof global.hybridd.asset[target][variable] !== 'undefined') {
        pdata = global.hybridd.asset[target][variable];
      }
      if(typeof global.hybridd.source[target] !== 'undefined' && typeof global.hybridd.source[target][variable] !== 'undefined') {
        pdata = global.hybridd.source[target][variable];
      }
      if(typeof global.hybridd.engine[target] !== 'undefined' && typeof global.hybridd.engine[target][variable] !== 'undefined') {
        pdata = global.hybridd.engine[target][variable];
      }
    // else store the variable in the master process
    } else {
 ...
 * @param {String} string - String to calculate outcome of.
 * @example
 *  hash("this_is_my_data")  // returns the hash: 5322fecfc92a5e3248a297a3df3eddfb9bd9049504272e4f572b87fa36d4b3bd
 */
  this.hash = function (p, input) {
    if (DEBUG) { console.log(' [D] (' + p.parentID + ') hash [' + input + '] in step ' + (global.hybridd.proc[p.parentID].step + 1) + ' of ' + (global.hybridd.proc[p.parentID].steps + 1)); }
    var hash = nacl.to_hex(nacl.crypto_hash_sha256(input));
    this.next(p, 0, hash);
  };  
  /**
 ...
 * @example
 *  sort('BADEFC')                                // sorts the object ascending by value: 'ABCDEF'
 *  sort([10,8,5,9],'desc')                       // sorts the object ascending by value: [5,8,9,10]
 *  sort({'a':'z','b':'x','c':'y'},['val','asc']) // sorts the object ascending by value: [{'b':'x'},{'c':'y'},{'c':'z'}]
 *  sort({'a':2,'b':1,'c':3},['key','desc'])      // sorts the object descending by key:  [{'c':3},{'b':1},{'a':2}]
 *  sort({'a':1,'b':10,'c':3},['val','asc'])      // sorts the object ascending by integer value:  [{'a':1},{'c':3},{'b':10}]
 *  sort([{'a':2,'b':1},{'a':4,'b':3}],['obj:b']) // sorts the array by object key 'b'
 */
  this.sort = function (p, object, method) {
    ...
    if(typeof method==='string') { method = (method!=='desc' && method!=='asc'?[method,'asc']:[method]); }
    ...
    for (var i = 0; i < method.length; i++) {
      if(method[i].substr(0,3)==='obj') {
        sortByObj = 1;
        methodCase = 'obj';
        objName = method[i].split(':')[1];
      } else {
        methodCase = method[i];
      }
      console.log(' ### '+methodCase);
      switch (methodCase) {
        case 'val':
          sortByKey = 0;
          sortByObj = 0;

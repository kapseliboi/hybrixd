...
function tranPropertyPath (p, path, data) { //  path: ["a","1"], data: {a:[1,2]}  => 2
  console.log('a', JSON.stringify(path));
  var iterator = data;
...
function mathCalc (equation, operator) {
 ...
 * @param {Number} input - Data to add to array
 * @example
 * push(0)          // add 0 to data array
 * push('woots!')   // add 'woots!' to data array
 */
  this.push = function (p, input) {
    if (DEBUG) { console.log(' [D] (' + p.parentID + ') collecting data into array'); }
    var ydata = global.hybridd.proc[p.processID].data;
 ...
 */
  ...
  /**
 * Grab data into array from previous subprocess steps.
 * @param {Number} steps - Number of previous steps to collate. Zero value means all previous steps.
 * @example
 * grab(-1)      // grab data of the previous step
 * grab(-5)      // grab data five steps back
 * grab([-1,-3]) // grab data from one step and three steps back, collated in an array
 */
  this.grab = function (p, steps) {
    if (DEBUG) { console.log(' [D] (' + p.parentID + ') grabbing subprocess data for ' + steps + ' steps'); }
    if (typeof steps === 'undefined') { steps = 0 - global.hybridd.proc[p.parentID].step; } // default: collate all previous steps
    var data = [];
    var curstep;
    if (Object.prototype.toString.call(steps) !== '[object Array]') {
      curstep = global.hybridd.proc[p.parentID].step + steps;
      if (typeof global.hybridd.proc[p.parentID + '.' + curstep] !== 'undefined') {
        data = global.hybridd.proc[p.parentID + '.' + curstep].data;
      }
    } else {
      for (var i = 0; i < steps.length; i += 1) {
        if (!isNaN(steps[i]) && steps[i] < global.hybridd.proc[p.parentID].steps) {
          curstep = global.hybridd.proc[p.parentID].step + steps[i];
          if (typeof global.hybridd.proc[p.parentID + '.' + curstep] !== 'undefined') {
            data.push(global.hybridd.proc[p.parentID + '.' + curstep].data);
          } else {
            data.push(null);
          }
        }
 ...
 * @param {Number} millisecs -  Amount of milliseconds to wait.
 * @param {Object} [data] - Set data when process times out
 * @param {Object} [err ] - Set error flag when process times out
 * @example
 ...
 */
  this.time = function (p, millisecs, hookData, hookErr) {
    ...
    for (var i = process.step; i <= process.steps; i += 1) {
      loopid = p.parentID + '.' + i;
    ...
    if (typeof hookData !== 'undefined' || typeof hookErr !== 'undefined') {
      process.timeHook = {data: hookData, err: (typeof hookErr === 'undefined' ? 1 : hookErr)};
    }
 ...
 * @param {String|Integer} onError - Determines fallback behaviour. String will st
 * @example
 ...
 */
  this.read = function (p, processIDs, millisecs) {
    ...
    if (permission) {
      ...
      var progress = 0;
      if (typeof processIDs === 'string') { // single
        finished = global.hybridd.proc[processIDs].progress >= 1 || global.hybridd.proc[processIDs].stopped > 0;
        ...
        for (var i = 0; i < processIDs.length; i++) {
          ...
          finished = finished + (global.hybridd.proc[processIDs[i]].progress >= 1 || global.hybridd.proc[processIDs[i]].stopped > 0);
        }
        finished = finished < processIDs.length ? 0 : 1;
        global.hybridd.proc[p.processID].progress = progress / processIDs.length;
      } else { // dictionary
        for (var key in processIDs) {
          var processID = processIDs[key];
          progress += global.hybridd.proc[processID].progress;
          finished = finished + (global.hybridd.proc[processID].progress >= 1 && global.hybridd.proc[processID].stopped > 0);
        }
        finished = finished < processIDs.length ? 0 : 1;
        global.hybridd.proc[p.processID].progress = progress / processIDs.length;
      }
 ...
 */
  ...
  /**
 * Reduce an object to an array with its parts.
 * @param {String} key - key to reduce object by
 * @example
 *  flat('.b')                 // input [{a:1,b:2,c:3},{a:2,b:2,c:3}], returns [2,2]
 *  alternative: tran('[].b')  // input [{a:1,b:2,c:3},{a:2,b:2,c:3}], returns [2,2]
 */
  this.flat = function (p, keystr) {
    var reduArray = global.hybridd.proc[p.processID].data;
    if (Object.prototype.toString.call(reduArray) !== '[object Array]') {
      reduArray = [scanArray];
    }
    var array = [];
    for (var key = 0; key < reduArray.length; key++) {
      array.push(eval('reduArray[key]' + String(keystr)));
    }
    this.next(p, 0, array);
  };
  /**
 ...
 */
  this.math = function (p, equation, operator) {
    ...
    this.next(p, 0, mathCalc(equation, operator));
  };
 ...
 * @param {Number} cinit      - Initial value of cumulative counter
 * @example

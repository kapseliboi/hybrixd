 ...
 */
  ...
  var readData = function (p, processIDs, millisecs, callback) {
 ...
 */
  this.with = function (p, variable, command, properties) {
    ...
    if (result.e > 0) {
      ...
      var subprocesses = [
        command.substr(0,4)+' '+args,
        "done",
      ];
      console.log(JSON.stringify(subprocesses));
      // fire the Qrtz-language program into the subprocess queue
      var processStepIdSplit = p.processID.split('.');
      var rootID = processStepIdSplit[0];
      var rootProcess = global.hybrixd.proc[rootID];
      var childPID = scheduler.init(0, {data: result.v, sessionID: rootProcess.sessionID, recipe: rootProcess.recipe});
      scheduler.fire(childPID, subprocesses);
      readData(p, childPID, 0, (p,err,xdata) => {
        if(err) {
          this.fail(p, err);
        } else {
          result = pokeVar(p, variable, xdata);
          if (result.e > 0) {

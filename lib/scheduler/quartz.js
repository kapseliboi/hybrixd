...
function func(p,module,funcname,xdata) {
  ...
  xdata.processID = p.processID;
  return ( new Function("modules.module."+module+".main."+funcname+"("+JSON.stringify(ydata)+");") )();
}

// go to next step
function next(p,err,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
function dump(p,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
  console.log("[D] ("+p.parentID+") dump: "+JSON.stringify(ydata));
  return next(p,0,ydata);
}

// wait a specified amount of time
function wait(p,millisecs,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
  ...
  return setTimeout(function(p,ydata) {
    next(p,0,ydata);
  },millisecs,p,ydata);
}
...
function logs(p,level,log,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
function jpar(p,xdata,failvalue) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
function prog(p,step,steps,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
function time(p,millisecs,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
var jump = function(p,stepto,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
function tran(p,property,testObject,is_valid,is_invalid,xdata){
  ...
  if(checkVar.valid){
    jump(p,is_valid,xdata?xdata:checkVar.data); // xdata instead of ydata is passed on purpose
  } else if(typeof is_invalid === "undefined") {
    next(p,0,typeof xdata!=="undefined"?xdata:testObject); // xdata instead of ydata is passed on purpose
  } else {
    jump(p,is_invalid,typeof xdata!=="undefined"?xdata:testObject); // xdata instead of ydata is passed on purpose
  }
  return 1;
}

// find a string in an array of objects, and return the object it was found in
function find(p,findObject,searchArray,found,not_found,xdata){
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
  ...
  if(matches.length>0) {
    ...
    jump(p,not_found,ydata);
  }
  return 1;
}

// format float number
function form(p,xdata,factor,convert){
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
// explode string
function expl(p,seperator,xdata){
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
// implode string
function impl(p,seperator,xdata){
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
...
function curl(p,target,querystring,method,xdata,headers,overwriteProperties){
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
  ...
  if(target.substr(0,8) === "asset://"){  // target = "asset://base.mode"
    ...
    if(targetSplitOnAdd.length==1){
      ...
      console.log("properties.host "+properties.host)
    }else{
...
function call(p,target,xpath,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
  read(p,fork(p,target,xpath,ydata,0));
}
...
function each(p,container,target,xpath,xdata) {
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;
  ...
  if(container instanceof Array){ // Array
    ...
    for(var key = 0; key < container.length; ++key){
      processIDs.push(fork(p,target,xpath, {data:ydata, container:container, key:key, value:container[key]},key));
    }
  }else{  // Dictionary
    processIDs = {};
    var index = 0;
    for(var key in container){
      processIDs[key] =fork(p,target,xpath, {data:ydata, container:container, key:key, value:container[key]},index);
      ++index;
    }
  }
  read(p,processIDs);
...
function poke(p,variable,pokedata,xdata){
  var ydata = typeof xdata === 'undefined'?global.hybridd.proc[p.processID].data:xdata;

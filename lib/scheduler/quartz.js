...
function test(p,condition,is_true,is_false,data) {
  ...
  if(condition) {
    jump(p,is_true,ydata);
  } else if(typeof is_false === "undefined") {
    next(p,0,ydata);
  } else {
    jump(p,is_false,ydata);
  }
  return 1;
}
...
function tran(p,property,testObject,is_valid,is_invalid,xdata){
  ...
  if(checkVar.valid){
    ...
    next(p,0,xdata?xdata:null); // xdata instead of ydata is passed on purpose
  } else {
    jump(p,is_invalid,xdata?xdata:testObject); // xdata instead of ydata is passed on purpose
  }
  return 1;
}

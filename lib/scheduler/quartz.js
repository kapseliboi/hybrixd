  ...
  this.hash = function (p, method) {
    ...
    switch (method) {
      ...
      case 'sha244':
        ...
        break;
      default:
        // WEIRD FORKING STUFF GOING ON HERE?
        //var SHA256 = ;
        //hashed = require('js-sha256').sha256(ydata);
        //console.log('  ## '+p.processID+' ## '+JSON.stringify(ydata)+'  '+hashed);
        // AVOID THESE BELOW!
        // NOT PARALLELIZEABLE DUE TO BUG(S) IN NACL: hash = nacl.to_hex(nacl.crypto_hash_sha256(global.hybrixd.proc[p.processID].data));
        var sha256 = require('../../common/crypto/sha256');
        var hash = nacl.to_hex(sha256(global.hybrixd.proc[p.processID].data))
        console.log('  ## '+p.processID+' ## '+JSON.stringify(ydata)+'  '+hash);
        break;
    }
    this.next(p, 0, hash);
  };

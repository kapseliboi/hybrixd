 ...
 * @example
 * coll(0)          // add 0 to data array
 * coll('woots!')   // add 'woots!' to data array
 */
  this.coll = function (p, input) {
 ...
 * @example
 * grab(0)     // grab data of step 0
 * grab(-5)    // grab data of the last five steps, collated in an array
 * grab([1,3]) // grab data from step 1 and 3, collated in an array
 */
  this.grab = function (p, steps) {
    ...
    if (Object.prototype.toString.call(steps) !== '[object Array]') {
      if (steps < 0) {
        ...
        if (typeof global.hybridd.proc[p.parentID + '.' + steps] !== 'undefined') {
          data = global.hybridd.proc[p.parentID + '.' + steps].data;
        }
      }
    } else {
      for (var i = 0; i < steps.length; i += 1) {
        if (!isNaN(steps[i]) && steps[i] > -1 && steps[i] < global.hybridd.proc[p.parentID].steps) {
          if (typeof global.hybridd.proc[p.parentID + '.' + steps[i]] !== 'undefined') {
            data.push(global.hybridd.proc[p.parentID + '.' + steps[i]].data);
          } else {
            data.push(null);
          }
        }

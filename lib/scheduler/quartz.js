...
var hybrixd = require('../../interface/hybrixd.interface.nodejs');
    ...
    if (permission) {
      ...
      if (typeof processIDs === 'string') { // single
        ...
        for (var key in processIDs) {
          ...
          finished += finished + (global.hybrixd.proc[processID].progress >= 1 || global.hybrixd.proc[processID].stopped) ? 1 : 0;
        }
 ...
 */
  this.exec = function (p, onSuccess, onError, xdata) {
    ...
    hybrixd.sequential(ydata, dataCallback, errorCallback, progressCallback);
  };
};

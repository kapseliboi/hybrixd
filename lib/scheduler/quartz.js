...
// used by with
function handleWithCmds (command, subprocesses) {
  if (typeof command === 'string') {
    ...
    if (command.substr(0, 4) === 'done') {
      ...
      subprocesses.push('jump ' + (Object.keys(arguments).length - i));
    } else {
    ...
    if (Object.keys(arguments).length > 10) {
      ...
      for (let j = 2; j < Object.keys(arguments).length; j++) {
        ...
        if (command[0].substr(0, 4) === 'done') {
          ...
          subprocesses.push('jump ' + (Object.keys(arguments).length - j));
        } else {
...
let Quartz = function () {
  ...
  */
   ...
   * meta("myFavoriteColor")  // retrieve the meta data for myFavoriteColor
   */
  this.seek = function (p, input, success, failure) {
    let key = input || global.hybrixd.proc[p.processID].data;

    const done = data => {
      if (data) {
        this.jump(p, isNaN(success) ? 1 : success || 1, global.hybrixd.proc[p.processID].data);
      } else {
        this.jump(p, isNaN(failure) ? 1 : failure || 1, global.hybrixd.proc[p.processID].data);
      }
    };
    modules.module['storage'].main.seek(
      {done}, {key: encodeURIComponent(key)}
    );
  };
    ...
    if (variable.substr(-2) === '[]') {
      ...
      if (result.v instanceof Array) {
        ...
        for (let k = 0; k < result.v.length; k++) {
          ...
          subprocesses = handleWithCmds(command, subprocesses);

 ...
 */
  this.code = function (p, source, target) {
    ...
    switch (source) {
      case 'base58':
        interim = new Buffer(require('base-x')(BASE58).decode(input)).toString();
        break;
      case 'base58num':
        encoder = require('base-x')(BASE58);
        interim = require('../../common/crypto/hex2dec').toDec( new Buffer(encoder.decode(input)).toString('hex') ).toInteger().toFixed().toString();
        break;
      case 'hex':
        interim = decodeURIComponent(input.replace(/(..)/g, '%$1'));
        break;
      case 'hexnum':
        interim = require('../../common/crypto/hex2dec').toDec( input ).toInteger().toFixed().toString();
        break;
      default:
        interim = input;
        break;
    ...
    switch (target) {
      ...
      case 'hex':
        ...
        break;
      case 'hexnum':
        output = require('../../common/crypto/hex2dec').toHex( interim ).substr(2);
        break;
      default:
        output = interim;
        break;

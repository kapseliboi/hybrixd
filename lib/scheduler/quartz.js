 ...
 */
  this.jpar = function (p, success, fail) {
    ...
    try {
      this.jump(p, isNaN(success)?1:success||1, JSON.parse(global.hybridd.proc[p.processID].data));
    } catch (e) {
      this.jump(p, isNaN(fail)?1:fail||1, global.hybridd.proc[p.processID].data);
    }
  };
 ...
 */
  this.test = function (p, condition, is_true, is_false) {
    ...
    if (condition) {
      this.jump(p, isNaN(is_true)?1:is_true||1, global.hybridd.proc[p.processID].data);
    } else if (typeof is_false === 'undefined') {
      this.next(p, 0, global.hybridd.proc[p.processID].data);
    } else {
      this.jump(p, isNaN(is_false)?1:is_false||1, global.hybridd.proc[p.processID].data);
    }
  };
  /**
 ...
 */
  this.true = function (p, condition, is_true, is_false) {
    if (DEBUG) { console.log(' [D] (' + p.parentID + ') truthing [' + condition + '] in step ' + (global.hybridd.proc[p.parentID].step + 1) + ' of ' + (global.hybridd.proc[p.parentID].steps + 1)); }
    if (typeof condition === 'string' && isNaN(condition.substr(0, 1))) {
      condition = global.hybridd.proc[p.processID].data+condition;
    }
 ...
 */
  this.flow = function (p, compare, success, failure) {
    ...
    if (typeof compare === 'string') {
      compare = eval('{"'+compare+'":"'+success+'"}');
      jumpTo = isNaN(failure)?1:failure||1;
    } else {
        jumpTo = isNaN(success)?1:success||1;
    }
    for (var key in compare) {
      if (global.hybridd.proc[p.processID].data===key) {
        jumpTo = compare[key];
      }
    }
    this.jump(p, jumpTo, global.hybridd.proc[p.processID].data);
  };
  /**
 ...
 */
  this.have = function (p, xdata) {
    var ydata = typeof xdata!=='undefined'?xdata:global.hybridd.proc[p.processID].data;
    if (DEBUG) { console.log(' [D] (' + p.parentID + ') have [' + ydata + '] in step ' + (global.hybridd.proc[p.parentID].step + 1) + ' of ' + (global.hybridd.proc[p.parentID].steps + 1)); }
    var result = 0;
 ...
 */
  this.poke = function (p, key, pdata) {
    ...
    if(typeof pdata !== 'undefined') {
      result.v = global.hybridd.proc[p.processID].data;
    }
    if(result.e>0) {
 ...
 */
  this.excl = function (p, value, success, failure) {
    var ydata = global.hybridd.proc[p.processID].data;
    if (DEBUG) { console.log(' [D] (' + p.parentID + ') excl [' + ydata + '] in step ' + (global.hybridd.proc[p.parentID].step + 1) + ' of ' + (global.hybridd.proc[p.parentID].steps + 1)); }
    found = 0;
    if (typeof ydata === 'string') {
      var str = true;
      ydata = ydata.split('');
 ...
 */
  this.loop = function (p,jumpTo,loopVar,condition,initVal,stepVal) {
    ...
    if(mathCalc(loopVal+condition)==='true') {
      ...
      this.jump(p, isNaN(jumpTo)?1:jumpTo||1, global.hybridd.proc[p.processID].data);
    } else {

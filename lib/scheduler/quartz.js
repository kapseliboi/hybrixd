  ...
  if (typeof key === 'string') {
    ...
    if (keySplit.length === 2) {
      ...
      if (scope === 'proc') { // proc::key
        ...
        var filePath = '../var/recipes/' + rootProcess.recipe.filename;
...
var Quartz = function () {
 /**
  ...
  this.func = function (p, funcname, xdata) { //TODO remove legacy xdata
    ...
    var rootProcess= global.hybrixd.proc[rootProcessID];
    var module = rootProcess.recipe.module;
    ...
    if(!modules.module.hasOwnProperty(module)){
      this.fail(p,`Unknown module '${module}'`);
    }else if(!modules.module[module].main.hasOwnProperty(funcname)){
      this.fail(p,`Unknown function '${funcname}' for module '${module}'`);
    }else{
      if(typeof xdata !== 'undefined'){
        ydata.processID = p.processID;
        modules.module[module].main[ funcname ](ydata);
      }else{
        var pass = data=>{this.pass(p,data);};
        var done = data=>{this.stop(p,data);};
        var stop = (err,data)=>{this.stop(p,err,data);};
        var fail = (err,data)=>{this.fail(p,err,data);};
        modules.module[module].main[ funcname ]({pass,done,stop,fail},ydata);
      }
    }
  };
  ...
  */
  ...
  this.fork = function (p, command, childId, xdata) {
    ...
    var main = modules.module[rootProcess.recipe.module].main;
  ...
  /**
   * TODO: Describe the functionality of file()
   * @example
   * rout('/asset/dummy/balance/__dummyaddress__')   // retrieve the balance for the dummy asset
   */
  ...
  this.save = function (p, key, value) {
    ...
    modules.module.storage.main.exec({processID:p.processID, command:['save',key,value]});
   ...
   */
  this.load = function (p, input) {
    var key = typeof input === 'undefined' ? global.hybrixd.proc[p.processID].data : input;
    if (DEBUG) { console.log(' [D] (' + p.parentID + ') load from storage '+key); }
    //modules.module.storage.main.exec({processID:p.processID, command:['load',key]});
    modules.module.storage.main.qrtzLoad({processID:p.processID,key:key});
  };
  ...
  this.meta = function (p, input) {
    ...
    modules.module.storage.main.exec({processID:p.processID, command:['meta',key]});
  ...
  this.poke = function (p, key, pdata) {
    ...
    var result = pokeVar(p, key, qdata);
  ...
  this.code = function (p, source, target) {
    ...
    var output = baseCode.recode (source,target,input);
  ...
  this.excl = function (p, value, success, failure) {
    ...
    var result = excludeEntries(ydata, value);
    if (str) {
      ydata = result.v.join('');
    } else {
      ydata = result.v;
  ...
  this.with = function (p, variable, command, properties) {
    ...
    if (typeof command === 'string') {
      var args = '';
      for (var i = 3; i < Object.keys(arguments).length; i++) {
      ...
      if (Object.keys(arguments).length > 10) {
        ...
        for (var j = 2; j < Object.keys(arguments).length; j++) {
    ...
    var childPID = scheduler.init(p.processID + '.', {data: result.v, sessionID: rootProcess.sessionID, recipe: rootProcess.recipe});
    scheduler.fire(childPID, subprocesses);
    readData(p, childPID, 0, (p, err, xdata) => {
      if (err) {
        this.fail(p, err);
      } else {
        var result = pokeVar(p, variable, xdata);
        if (result.e > 0) {
   ...
   * keys secret   // Recreate the public key from a secret key.
   */
  this.keys = function (p, secret) {
    if (typeof secret === 'undefined') {
      ...
      this.next(p, 0, {public,secret}); // create keypair
    } else if(typeof key === 'string' && key.length === 128){
      secret = nacl.from_hex(secret);
      var keys = nacl.sign.keyPair.fromSecretKey(secret).publicKey;
      var public = nacl.to_hex(keys.signPk);
      var secret = nacl.to_hex(keys.signSk);
      this.next(p, 0, {public,secret}); //  public key from secret key
    }else{
  ...
  this.sign = function (p, key, signatureOrOnSuccess, onSuccessOrOnFail, onFail) {
    var message =global.hybrixd.proc[p.processID].data;
    if (typeof message === 'string' && typeof key === 'string' && key.length === 64) {
      var public = nacl.from_hex(key);
      message = nacl.from_hex(message);
      if (typeof signatureOrOnSuccess === 'string') {
        var verified = nacl.crypto_sign_verify_detached(message, signatureOrOnSuccess, public);
        if(verified){
  ...
  this.regx = function (p, pattern, onMatch, onNoMatch) {
    ...
    const pattern2 = pattern;//pattern.replace(new RegExp('^/(.*?)/'+flags+'$'), '$1');

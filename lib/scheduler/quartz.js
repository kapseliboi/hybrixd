...
let Quartz = function () {
   ...
   * }
   */
  this.func = function (p, funcname, xdata) { // TODO remove legacy xdata now used by ethereum module
    ...
    if (!modules.module.hasOwnProperty(moduleID)) {
      ...
      if (typeof xdata !== 'undefined') { // TODO legacy mode. DEPRICIATED
        ...
        const pass = data => { this.pass(p, data); };
        const done = data => { this.stop(p, 0, data); };
        const stop = (err, data) => { this.stop(p, err, data); };

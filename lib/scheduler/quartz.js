...
let JSONfix = require('../util/jsonfix').parse;
...
let Mathjs = require('../../common/crypto/mathjs.js');
Mathjs.config({
...
// used by with
function handleWithCmds (command, subprocesses, argsArray) {
...
// used by atom
let toInt = function (input, factor) {
  let f = Number(factor);
  let x = new Decimal(String(input));
...
// used by atom
let fromInt = function (input, factor) {
  let f = Number(factor);
  let x = new Decimal(String(input));
  ...
  if (typeof equation === 'string' && equation.length > 0) {
    ...
    try {
      if (isFunctionRegex.test(equation) === true) {
        ...
        result = stricteval('Mathjs.' + equation + '.toString();');
      } else {
        result = Mathjs.eval(equation).toString();
      }
    } catch (e) {
...
let Quartz = function () {
   ...
   * }
   */
  this.func = function (p, command, xdata) { // TODO remove legacy xdata now used by ethereum module
  ...
  */
  ...
  this.logs = function (p, level) {
    ...
    if (messages.length === 0) { messages.push(global.hybrixd.proc[p.processID].data); }
  ...
  this.prog = function (p, start, finish, milliseconds) {
    if (typeof start === 'undefined') {
      ...
      if (!milliseconds) {
        ...
        let increment = (finish - start) / (milliseconds * 0.002);
        let progressvalue;
        global.hybrixd.proc[p.parentID].progress = start;
   ...
   */
  this.mime = function (p, type) {
    scheduler.mime(p.processID, type);
    this.next(p, 0, global.hybrixd.proc[p.processID].data);
  };
  ...
  this.true = function (p, condition, is_true, is_false) {
    if (typeof condition === 'string' && isNaN(condition.substr(0, 1))) {
      ...
      condition = global.hybrixd.proc[p.processID].data;
    }
   ...
   */
  this.void = function (p, compare, success, failure) {
    let ydata = global.hybrixd.proc[p.processID].data;
    let jumpTo = success || 1;
  ...
  this.tran = function (p, property, is_valid, is_invalid) {
    const ydata = global.hybrixd.proc[p.processID].data;
   ...
   */
  this.have = function (p, property, is_valid, is_invalid) {
    const ydata = global.hybrixd.proc[p.processID].data;
    const checkVar = chckVar(p, property, ydata);
    if (checkVar.valid) {
  ...
  this.form = function (p, factor) {
    ...
    let result = padFloat(global.hybrixd.proc[p.processID].data, factor);
    this.next(p, 0, result);
  };
   ...
   * @param {Object} [onSuccess=1] -  Amount of instructions lines to jump on success.
   * @param {Object} [onFail=1] -  Amount of instructions lines to jump on failure.
   */
  this.curl = function (p, target, querystring, method, headers, overwritePropertiesOrOnSuccess, onSuccessOrOnFail, onFailOrNothing) {
    let jumpOnSuccess;
    let jumpOnFailure;
   ...
   * pack
   */
  this.pack = function (p) {
    let ydata = LZString.compressToEncodedURIComponent(JSON.stringify(global.hybrixd.proc[p.processID].data));
    this.next(p, 0, ydata);
  };
   ...
   * unpk
   */
  this.unpk = function (p, success, failure) {
    let ydata, jumpTo;
    try {
      ydata = JSON.parse(LZString.decompressFromEncodedURIComponent(global.hybrixd.proc[p.processID].data));
      jumpTo = success || 1;
    } catch (e) {
      ydata = global.hybrixd.proc[p.processID].data;
      jumpTo = failure || 1;
   ...
   * rout('/asset/dummy/balance/__dummyaddress__',@success,@notFound)   // retrieve the balance for the dummy asset
   */
  this.rout = function (p, xpath, success, failure) {
    ...
    if (result.id === 'id') { // if result is a process that has te be awaited, then read that proces until finished
      readData(p, result.data, 0, (p, err, xdata) => {
        if (err) {
  ...
  this.save = function (p, key, value) {
    ...
    const done = data => { this.next(p, 0, global.hybrixd.proc[p.processID].data); };
    const fail = (err, data) => { this.fail(p, err, 'Cannot save!'); };
    if (typeof value !== 'string') {
      value = JSON.stringify(value);
    }
    modules.module['storage'].main.save(
      {done, fail}, {key: encodeURIComponent(key), value: value, noSync: true}
    );
  };
  ...
  this.load = function (p, input, success, failure) {
    ...
    const fail = () => { this.jump(p, isNaN(failure) ? 1 : failure || 1, global.hybrixd.proc[p.processID].data); };
    modules.module['storage'].main.load(
      {done, fail}, {key: encodeURIComponent(key), readFile: true}
    );
  };
   ...
   */
  this.meta = function (p, input, success, failure) {
    let key = input || global.hybrixd.proc[p.processID].data;

    const done = data => { this.jump(p, isNaN(success) ? 1 : success || 1, data); };
    const fail = () => { this.jump(p, isNaN(failure) ? 1 : failure || 1, global.hybrixd.proc[p.processID].data); };
    modules.module['storage'].main.meta(
      {done, fail}, {key: encodeURIComponent(key)}
    );
  };
   ...
   */
  this.seek = function (p, key, success, failure) {
    key = key || global.hybrixd.proc[p.processID].dataglobal.hybrixd.proc[p.processID].data; // TODO handle success  and failure in this case
    const done = data => { this.jump(p, isNaN(success) ? 1 : success || 1, data); };
    const fail = () => { this.jump(p, isNaN(failure) ? 1 : failure || 1, global.hybrixd.proc[p.processID].dataglobal.hybrixd.proc[p.processID].data); };
    modules.module['storage'].main.seek({done, fail}, {key: encodeURIComponent(key)});
  };
   ...
   */
  this.burn = function (p, key, success, failure) {
    key = key || global.hybrixd.proc[p.processID].dataglobal.hybrixd.proc[p.processID].data; // TODO handle success  and failure in this case
    const done = data => { this.jump(p, isNaN(success) ? 1 : success || 1, data); };
    const fail = () => { this.jump(p, isNaN(failure) ? 1 : failure || 1, global.hybrixd.proc[p.processID].data); };
    modules.module['storage'].main.burn({done, fail}, {key: encodeURIComponent(key)}
    );
  };
   ...
   */
  this.poke = function (p, key, pdata, ddata) {
    let qdata;

    if (typeof pdata === 'undefined') {
      if (typeof ddata === 'undefined') {
        qdata = global.hybrixd.proc[p.processID].data;
      } else {
        qdata = ddata;
      }
    } else {
  ...
  this.peek = function (p, key) {
    const qdata = global.hybrixd.proc[p.processID].data;
    ...
    if (key instanceof Array && qdata instanceof Array) {
      if (key.length === qdata.length) {
        const values = [];
        for (let index = 0; index < key.length; ++index) {
          const result = vars.peek(p, key[index]);
          if (result.e > 0) {
            return this.fail(p, result.v);
          } else {
            values.push(result.v);
   ...
   */
  this.shuf = function (p) {
    let ydata = global.hybrixd.proc[p.processID].data;
    let result = null;
    if (ydata instanceof Array) {
      result = shuffle(ydata);
    }
    this.next(p, 0, result);
  };
   ...
   */
  this.copy = function (p, variable, properties) {
    let result;
  ...
  this.with = function (p, variable, command, properties) {
    ...
    if (variable.substr(-2) === '[]') {
      ...
      if (result.v instanceof Array) {
        ...
        for (let k = 0; k < result.v.length; k++) {
          ...
          subprocesses = handleWithCmds(command, subprocesses, arguments);
    ...
    readData(p, childPID, 0, (p, err, xdata) => {
      if (err) {
        ...
        let result = vars.poke(p, variable, xdata);
        if (result.e > 0) {

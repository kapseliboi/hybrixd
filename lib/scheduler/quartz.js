 ...
 */
  this.load = function (p, key) {
    ...
    // TODO make sure that value is decoded as well (pass postprocess callback to rout and read?)
  };
  ...
  this.code = function (p, source, target) {
    ...
    switch (source) {
      ...
      case 'bin':
        ...
        break;
      default:
        if (source === 'ascii' || source === 'lowercase' || source === 'uppercase' || source === 'utf-8' || source === 'utf8') { source = BASE(256); }
    ...
    switch (target) {
      ...
      default:
        if (target === 'ascii' || target === 'lowercase' || target === 'uppercase' || target === 'utf-8' || target === 'utf8') { target = BASE(256); }
    ...
    switch (postTarget) {
      case 'lowercase': output = output.toLowerCase(); break;
      case 'uppercase': output = output.toUpperCase(); break;
      default: break;
    }    
 ...
 * push('B',8)      // input: ['a','c'], result: ['a','B','c']
 ...
 * @param {String} string - String to delete from

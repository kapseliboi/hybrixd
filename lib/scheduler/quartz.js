 ...
 */
  this.jpar = function (p, success, fail) {
    ...
    try {
      this.jump(p, success || 1, JSON.parse(global.hybridd.proc[p.processID].data));
    } catch (e) {
      this.jump(p, fail || 1, global.hybridd.proc[p.processID].data);
    }
  };
 ...
 */
  this.test = function (p, condition, is_true, is_false) {
    ...
    if (condition) {
      this.jump(p, is_true || 1, global.hybridd.proc[p.processID].data);
    ...
    if (offset instanceof Array) {
      ...
      if (!isNaN(offset)) {
        ...
        if((ending-offset)<2) {
          ...
          result = [];
          for (var key = offset; key < ending+1; key++) {

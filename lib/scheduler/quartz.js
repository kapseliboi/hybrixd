...
let functions = require('../functions'); // only required for   procinfo[0] = functions.implode('.', pieces);
...
let Quartz = function () {
  ...
  this.func = function (p, funcname, xdata) { // TODO remove legacy xdata
    ...
    let module = rootProcess.recipe.module;
  ...
  */
  ...
  this.type = function (p, type) {
    let ydata = typeof type === 'undefined' ? global.hybrixd.proc[p.processID].data : type;
  ...
  this.tran = function (p, property, is_valid, is_invalid) {
    ...
    let testObject = global.hybrixd.proc[p.processID].data;
    let checkVar = chckVar(p, property, testObject);
    if (checkVar.valid) {
  ...
  this.find = function (p, findObject, found, not_found) {
    ...
    let searchArray = global.hybrixd.proc[p.processID].data;
    let matches = [];
    let query;
  ...
  this.math = function (p, equation, operator) {
    let data = global.hybrixd.proc[p.processID].data;
    let equationChar = equation.trim().substr(0, 1);

 ...
 */
  ...
  /**
 * TODO
 * @param {Object} onError - Object to fill the data stream with.
 * @example
 * data("test")            // Fill the data stream with 'test'.
 */
  this.emit = function (p, onError) {
    var parentProcess = global.hybridd.proc[p.parentID];
    parentProcess.onFail = typeof onError === 'string' ? onError : (onError + parentProcess.step);
    this.next(p, 0, global.hybridd.proc[p.processID].data);
  };
  /**
 ...
 */
  this.math = function (p, equation, operator) {
    ...
    if (Object.prototype.toString.call(equation) === '[object Array]') {
      ...
      equation = String(data)+String(equation);
    }
 ...
 */
  this.scan = function (p, test, cumulative, cinit) {
    ...
    // TODO: Decimalize input test
    if(typeof cinit==='undefined') { cinit=0; } else { cinit=mathCalc('0+'+cinit); }
    // TODO: make sum of data input values
    // TODO: handle sum using math()
    var val;
    var cnt = '0';
    var out;
    var matches=[];

  ...
  */
  ...
  this.flow = function (p, compare, success, failure) {
    ...
    for (var key in compare) {
      if (prefix + String(global.hybrixd.proc[p.processID].data) === key) {
        jumpTo = compare[key];
      }
    }
  ...
  this.file = function (p,success,failure) { // todo pass offest and length
    ...
    if (process.type.startsWith('file:')) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        this.jump(p, 0, isNaN(success) ? 1 : success || 1, processStep.data);
      } else {
        this.jump(p, 1, isNaN(failure) ? 1 : failure || 1, processStep.data);
      }
    } else {
      this.jump(p, 0, isNaN(failure) ? 1 : failure || 1, processStep.data);
    }
  };
  ...
  this.load = function (p, input, success, failure) {
    ...
    var filePath = '../storage/' + fold + key.replace('..', '');
    if (fs.existsSync(filePath)) {

...
let Quartz = function () {
  ...
  */
   ...
   */
  this.curl = function (p, target, querystring, method, headers, overwriteProperties/* overwritePropertiesOrOnSuccess, onSuccessOrOnFail, onFailOrNothing */) {
    /* let onSuccess = 1;
    let onFail = 1;
    ...
    if (typeof overwritePropertiesOrOnSuccess !== 'undefined') {
      if (isNaN(overwritePropertiesOrOnSuccess)) {
        ...
        onSuccess = onSuccessOrOnFail || 1;
        onFail = onFailOrNothing || 1;
      } else {
        onSuccess = overwritePropertiesOrOnSuccess || 1;
        onFail = onSuccessOrOnFail || 1;
      }
    } */
  ...
  this.scan = function (p, test, cumulative, cinit) {
    ...
    for (let key = 0; key < scanArray.length; key++) {
      ...
      teststr = test.replace(/(key|value|val|cnt)[.|a-z|A-Z]*/g, function (x) { return (+eval(x)).toFixed(17); });

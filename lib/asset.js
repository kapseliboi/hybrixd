...
function process(request,xpath) {
	...
	if (xpath.length == 1) {
      ...
      // possible asset functions
			var paths_assetexec = ['address','balance','contract','factor','fee','history','push','status','test','transfer','unspent'];
...
function assetlist() {
  ...
  for (var key in global.hybridd.asset) {
	...
	return {error:0, info:'List of available assets and their modes.', id:'asset', count:assetcnt, data:functions.sortObjectByKey(asset)};
}

function assetexec(target,lxpath,sessionID) {
	// init new process
	var processID = scheduler.init(0,{sessionID:sessionID});
  ...
  // run the module connector function - disconnects and sends results to processID!
    ...
    modules.module[global.hybridd.asset[target.symbol].module].main.exec({processID,target,command});
...
function directcommand(target,lxpath,sessionID) {
	if(lxpath[3]) {
      ...
      if(typeof global.hybridd.asset[target.symbol].mode != 'undefined') { var mode = global.hybridd.asset[target.symbol].mode; } else { mode = null; }
      if(typeof global.hybridd.asset[target.symbol].type != 'undefined') { var type = global.hybridd.asset[target.symbol].type; } else { type = null; }
      if(typeof global.hybridd.asset[target.symbol].factor != 'undefined') { var factor = global.hybridd.asset[target.symbol].factor; } else { factor = 8; }

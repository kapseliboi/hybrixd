...
function request (data, dataCallback, errorCallback) {
  ...
  closeHandler = (err) => {
    console.log(' [!] tcp closed for ' + host + ':', err);
    clearListners();
    errorCallback(err);
  };

  errorHandler = (err) => {
    console.log(' [!] tcp error for ' + host + ':', err);
    clearListners();
    client.destroy();
    errorCallback(err);
  };
  ...
  timeoutHandler = (err) => {
    console.log(' [!] tcp timeout for ' + host + ':', err);
    clearListners();
    client.destroy();
    errorCallback(err);
  };
        ...
        try { // try to parse the data
          ...
          if (result.hasOwnProperty('id')) {
            if (result.id === data.data.id) {
              ...
              clearListners();
              client.destroy();
              console.log(` [!] tcp error : response id ${result.id} doest not match request id ${data.data.id}`);
              errorCallback('Wrong result');
            }
          }
        } catch (e) {
          console.log(' [!] tcp JSON object parse error : ' + e + ' for ' + data);
          clearListners();
          client.destroy();
          errorCallback(e);
        }

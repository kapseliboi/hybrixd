        ...
        default:
          if (typeof data === 'string') {
            try {
              ...
              // DEBUG: console.log(' [i] API queue: no JSON, skipping parsing... ');
            }
          }
          break;
      }
    } catch (result) {
      console.log(' [!] API queue: parsing failed: (/proc/' + APIrequest.qrtzProcessStep.getProcessID() + ') ' + data);
...
// console.log(' [i] API queue: jump error:' + e));
}
...
function fail (qid, data) {
  ...
  if (testMode && !testModeForce && testRead(APIrequest, qid)) {
    ...
    // e => console.log(' [i] API queue: jump error:' + e)
  }
}

...
function mkTcpLink (host) {
  const [protocol, hostNameAndPort] = host.split('://');
  let [hostName, port] = hostNameAndPort.split(':');
  port = port || defaultProtocolPorts.tcp;
  port = Number(port);
  const link = new tcp.Link(protocol, hostName, port);
  return link;
}
...
function mkLink (APIrequest, host) {
  ...
  if (host.startsWith('ws://') || host.startsWith('wss://')) {
    link = mkWsLink(host);
  } else if (host.startsWith('tcp://') || host.startsWith('tcps://')) {
    link = mkTcpLink(host);
...
function call (host, APIrequest, now, dataCallback, errorCallback) {
  ...
  if (!link) {
    errorCallback('Failed to create link');
  } else {
    ...
    try {
      ...
      if (host.startsWith('tcp://') || host.startsWith('tcps://')) {

...
const teletype = require('./teletype_modified');
...
function tcpCall (link, host, qpath, args, method, dataCallback, errorCallback) {
  link.send(args.data, dataCallback, errorCallback);
}
...
function httpCall (link, host, qpath, args, method, dataCallback, errorCallback) {
  ...
  const args_ = (typeof args !== 'object' || args === null) ? {} : args;

  if (args_ && args_.headers && methodLowerCase === 'post') {
    args_.headers['content-length'] = args_.data // to handle a bug in Node JS which makes the server vulnerable to DDOS
...
function mkTcpLink (host) {
  ...
  const link = teletype(hostaddr, hostport);
  link.read(error => { console.log(' [!] failed to initiate tcp link for ' + host); });
  return link;
}

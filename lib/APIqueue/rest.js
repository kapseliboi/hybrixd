  ...
  const Util = {
    ...
    createProxyHeaders: function (url) {
      ...
      if (self.proxy.user && self.proxy.password) {
        result['Proxy-Authorization'] = 'Basic ' + Buffer.from([self.proxy.user, self.proxy.password].join(':')).toString('base64');
      }
...
util._extend(ConnectManager, events.EventEmitter.prototype); // TODO FIXME

...
const proxy = function (options, dataCallback, errorCallback) {
  ...
  proxyTunnel.on('connect', (res, socket, head) => {
    ...
    const request = protocol.request(options, res => {
      ...
      res.on('end', function () {
        handleEnd(options, res, buffer, dataCallback, errorCallback);
      });
...
const Client = function (options) {
  ...
  this.createConnectOptions = function (connectURL, connectMethod, args) {
    ...
    if (this.connectionthis && typeof this.connection === 'object') {
      for (const option in this.connection) {
    ...
    if (!this.useProxyTunnel) {
      for (const option in result.proxy) {
  ...
  this.connect = function (method, url, args, dataCallback, errorCallback) {
    ...
    const options = this.createConnectOptions(this.parsePathParameters(args, url), method, args);
    ...
    if (typeof args === 'object') {
      ...
      if (args.data !== undefined && (method === 'POST' || method === 'PUT' || method === 'PATCH')) {

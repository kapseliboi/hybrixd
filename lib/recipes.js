...
function mergeQuartz (target, source) {
  ...
  for (let sourceFunctionSignature in source.quartz) {
    const sourceFunctionName = sourceFunctionSignature.split('/')[0];
    let functionFound = false;
    for (let targetFunctionSignature in target.quartz) {
...
function overwriteLocalDelta (fileName, recipe) {
  ...
  if (fs.existsSync(filePath)) { // handle local recipe delta
    try {
      ...
      for (let key in delta) recipe[key] = delta[key]; // overwrite defaults with local property
    } catch (e) {
...
function parseRecipe (fileName, filePath) {
  ...
  global.hybrixd.logger(['info', 'recipes'], `Found ${recipeType} recipe ${fileName}`);
}

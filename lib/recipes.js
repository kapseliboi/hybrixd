...
function compileRecipe (recipe) {
  return function (id) {
    ...
    } else if (recipe === 'source' || recipe === 'engine') {
      setGlobalRecipe(recipe, id);
    }
  };
}

function setGlobalRecipe (recipe, id) {
  if (global.hybrixd[recipe][id].hasOwnProperty('router')) { // load router data for engines
    global.hybrixd.routetree[recipe][id] = global.hybrixd[recipe][id].router;
  }
}

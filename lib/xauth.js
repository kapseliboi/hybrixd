...
function xauth(http_req,xpath) {
	...
	if ( typeof global.hybridd.xauth.session[session_hexkey].nonce1 != "undefined" && session_step == "1" ) {
		...
		global.hybridd.xauth.session[session_hexkey].nonce1 = global.hybridd.xauth.session[session_hexkey].nonce1.replace(/^[8-9a-f]/,(match) => { var range=["8","9","a","b","c","d","e","f"]; return range.indexOf(match); });
		...
		if ( crypt_msg ) {
			...
			if ( secrets_json.nonce1 == global.hybridd.xauth.session[session_hexkey].nonce1 ) {
				...
				// TODO: report public key for security downgrade (nonce1 does not match)
			}
		}			
	}

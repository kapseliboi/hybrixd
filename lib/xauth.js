    ...
    try {
      ...
      if ( typeof global.hybridd.xauth.session[session_hexkey] === "undefined" && session_step === 0 && session_hexkey.length === 64) { // no session defined	
        ...
        if ( crypt_msg ) {
          ...
          //DEBUG 
          console.log(JSON.stringify(secrets_json));  // works!
          ...
          if ( secrets_json.nonce1 === global.hybridd.xauth.session[session_hexkey].nonce1 ) {
            ...
            // DEBUG: 
            //console.log('Server side xcrypt nonce: '+nonce_conhex+' CryptPacket data: '+JSON.stringify(session_secrets)); // this works

            // using signing method
            var crypt_bin = nacl.encode_utf8(JSON.stringify(session_secrets));
            //DEBUG
            //console.log('Server side utf-8'+crypt_bin); // this works
            var sign_hex = nacl.to_hex( nacl.crypto_sign(crypt_bin,server_session_signpair.signSk) );
            //DEBUG
            console.log('Server side sign_hex'+sign_hex); // works
            ...
            //console.log('Server side crypt_hex'+crypt_hex); // works
            ...
            //console.log("xresponse:"+JSON.stringify(xresponse)); // works
          }
        }			
      } else {
